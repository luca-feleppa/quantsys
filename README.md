...
- **Linea VOL — forecasting (produzione, validata).** Target `log_rv` (log realized variance su h barre): unico segnale che generalizza OOS, batte **HAR-CJ** (baseline con decomposizione continua/salti) del 32% in QLIKE su test. Giudicata con QLIKE, **mai** tradata nel backtest direzionale.
...
- **VOL line — forecasting (production, validated).** Target `log_rv` (log realized variance over h bars): the only signal that generalizes OOS, beating **HAR-CJ** (continuous/jump-decomposition baseline) by 32% in test QLIKE. Judged via QLIKE, **never** traded in the directional backtest.
