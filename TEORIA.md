...
🇮🇹 **Provenienza del numeratore della banda pubblicata — CHIARITA (2026-08-01), e con essa un limite di riproducibilità che va dichiarato.** La banda **−23% ÷ −32%** ha come numeratore la QLIKE di una **coppia modello/scaler RIADDESTRATA** sull'npz corrente (val **0.26143**, test **0.23637**), non quella del checkpoint congelato in `models/itransformer`, che è il **restore del PASS di giugno**. La distinzione non è pedanteria: sullo stesso identico campione il checkpoint di giugno dà val **0.27470** e test **0.24453**, cioè rapporti NN/HAR-C di 0.8152 e 0.7071 (−18.5% e −29.3%) invece di 0.7758 e 0.6835. **Il campione non c'entra:** `n`, HAR-RV, HAR-C, HAR-CJ e la naive coincidono **cifra per cifra** fra tutti i report recenti (val n=6485, test n=6486) — a variare è **solo** il modello. ⚠ **La ragione per cui il checkpoint di giugno NON è il numeratore giusto è metodologica, non di comodo:** il suo `PipelineState` porta `target_scale = 1.4375922` e un RobustScaler diverso (md5 di `center_`/`scale_` distinti) da quello canonico dell'npz corrente (`target_scale = 1.4268685`). Valutarlo sul dataset attuale significa dargli input normalizzati con uno scaler e denormalizzarne μ con un altro: la differenza che ne esce è in parte **artefatto di scaler**, non skill — la stessa classe dell'A8 −4.94% che a parità di scaler valeva −0.79% (§12.4). Una coppia riaddestrata è addestrata **e** valutata dentro lo stesso vintage, quindi è l'unico confronto pulito. ⚠ **Limite dichiarato:** la sandbox che ha prodotto il numeratore è stata **eliminata** al ripristino dello stato production (passo 5 del protocollo), quindi la banda **non è riproducibile alla cifra**: ripetere il protocollo dà un numero vicino ma non identico (una seconda coppia riaddestrata sullo stesso npz ha dato val 0.26206 anziché 0.26143, cioè rapporto 0.7777 contro 0.7758 — **~0.2 punti di incertezza di seed-draw**, che è la precisione onesta con cui vanno letti gli estremi della banda; ⚠ il rapporto della seconda replica è stato **corretto il 2026-08-03**, la versione precedente riportava 0.7738, che è il rapporto della *prima* replica contro **HAR-CJ** e non quello della seconda contro **HAR-C** — l'entità dello spread non cambia). **Conseguenza per chi legge:** la banda è un'affermazione sul **protocollo di addestramento** applicato al dataset corrente, non sul singolo file di checkpoint presente in `models/`; e i campi `metrics.nn` di `results/vols/qlike_report_1h_{val,test}_c3.json` **non sono** il numeratore della banda e non vanno usati come tali. **Correzioni applicate perché non ricapiti:** (i) `dev_vols_qlike.py` **fail-fast** se lo scaler del modello ≠ scaler canonico dell'npz, con via di fuga esplicita `--allow-scaler-mismatch` che marca il report come non confrontabile (guard visto fallire proprio sul modello di produzione); (ii) ogni report porta ora un blocco **`provenance`** con model dir, arch, npz e impronta dei due scaler — un numero non può più restare orfano del modello che l'ha prodotto; (iii) `tests/test_scaler_identity_guard.py`. **Non risolto e fuori da questa nota:** ripristinare la riproducibilità alla cifra richiede di addestrare una coppia canonica sull'npz corrente e di **ri-pubblicare la banda** con quei numeri — è una decisione sul claim, non una correzione, e non è stata presa qui. **Aggiornamento 2026-08-03: la pre-registrazione esiste ed è APERTA, MAI eseguita** (gate **R1** in `STATUS.md`): coppia canonica addestrata a config di produzione invariata su npz congelato, con quattro condizioni pre-dichiarate — controllo di vintage model-independent sulle baseline, identità dello scaler (`matches: true`), sopravvivenza del gate storico contro **HAR-RV** (`≤ 0.95·QLIKE_HAR`) e soglia di **materialità** `NN/HAR-C ≤ 0.80` sotto cui la banda resta invariata alla cifra pubblicata. Vi è dichiarato ex-ante anche che la coppia canonica **non viene promossa** in `models/itransformer` né sul VPS finché i campioni forward pre-registrati sono aperti: cambiare il modello dentro un campione è la stessa violazione del refresh macro dentro un campione.

**EN** **Provenance of the published band's numerator — CLARIFIED (2026-08-01), together with a reproducibility limit that must be declared.** The **−23% to −32%** band takes as numerator the QLIKE of a **RETRAINED model/scaler pair** on the current npz (val **0.26143**, test **0.23637**), not that of the frozen checkpoint in `models/itransformer`, which is the **June PASS restore**. The distinction is not pedantry: on the very same sample the June checkpoint yields val **0.27470** and test **0.24453**, i.e. NN/HAR-C ratios of 0.8152 and 0.7071 (−18.5% and −29.3%) instead of 0.7758 and 0.6835. **The sample is not the cause:** `n`, HAR-RV, HAR-C, HAR-CJ and naive match **digit for digit** across all recent reports (val n=6485, test n=6486) — **only** the model varies. ⚠ **The reason the June checkpoint is NOT the right numerator is methodological, not convenient:** its `PipelineState` carries `target_scale = 1.4375922` and a different RobustScaler (distinct `center_`/`scale_` md5) from the current npz's canonical one (`target_scale = 1.4268685`). Evaluating it on today's dataset means feeding it inputs normalized under one scaler and denormalizing μ against another: part of the resulting difference is a **scaler artifact**, not skill — the same class as A8's −4.94%, which at equal scaler was −0.79% (§12.4). A retrained pair is trained **and** evaluated inside the same vintage, so it is the only clean comparison. ⚠ **Declared limit:** the sandbox that produced the numerator was **deleted** when the production state was restored (protocol step 5), so the band is **not digit-reproducible**: repeating the protocol gives a nearby but different number (a second pair retrained on the same npz gave val 0.26206 rather than 0.26143, i.e. ratio 0.7777 against 0.7758 — **~0.2 points of seed-draw uncertainty**, which is the honest precision for reading the band's endpoints; ⚠ the second replica's ratio was **corrected on 2026-08-03**: the previous version read 0.7738, which is the *first* replica's ratio against **HAR-CJ**, not the second's against **HAR-C** — the size of the spread is unchanged). **Consequence for the reader:** the band is a statement about the **training protocol** applied to the current dataset, not about the individual checkpoint file sitting in `models/`; and the `metrics.nn` fields of `results/vols/qlike_report_1h_{val,test}_c3.json` **are not** the band's numerator and must not be used as such. **Fixes applied so it cannot recur:** (i) `dev_vols_qlike.py` **fails fast** when the model scaler ≠ the npz's canonical scaler, with an explicit `--allow-scaler-mismatch` escape that flags the report as non-comparable (the guard was seen failing on the production model itself); (ii) every report now carries a **`provenance`** block with model dir, arch, npz and both scaler fingerprints — a number can no longer be orphaned from the model that produced it; (iii) `tests/test_scaler_identity_guard.py`. **Unresolved and out of scope here:** restoring digit reproducibility requires training a canonical pair on the current npz and **re-publishing the band** with those numbers — a decision about the claim, not a correction, and it was not taken here. **Update 2026-08-03: the pre-registration exists and is OPEN, NEVER run** (gate **R1** in `STATUS.md`): a canonical pair trained at unchanged production config on a frozen npz, with four pre-declared conditions — a model-independent vintage check on the baselines, scaler identity (`matches: true`), survival of the historical gate against **HAR-RV** (`≤ 0.95·QLIKE_HAR`), and a **materiality** threshold `NN/HAR-C ≤ 0.80` below which the band stays unchanged at published precision. It also declares ex ante that the canonical pair is **not promoted** into `models/itransformer` nor onto the VPS while pre-registered forward samples are open: swapping the model inside a sample is the same violation as refreshing macro inside a sample.
