...
| B4 | ✅ **FATTO 2026-07-23 come B4-bis** — npz ri-derivato (`dev_vols_dvol_append.py`, X_macro 90→93, copertura val 1.000), pre-reg `5a6112d` committata PRIMA di ogni numero, run baseline+candidato in sandbox | `dev_vols_qlike.py` su val | **ESITO: FAIL su val** — candidato 0.25939 vs baseline riaddestrata 0.26206 = **−1.02%**, soglia −3% → ① FAIL (② sarebbe passata, gate in AND), nessun one-shot su test. Il prior onesto era corretto: tenor mismatch 30d→30h + incumbent che già cattura la IV via lag RV (MSE-log −6% ma QLIKE no). **Filone DVOL-come-feature chiuso** (close `526659d`) |
...
| C3 | ✅ **FATTO 2026-07-18** — servizio VPS riavviato con `--execute --hedge --hedge-band 0.30 --hedge-conv raw` (congelati A4; NO ww) → **forward v2 PARTITO**; posizione parzialmente hedgiata ESCLUSA dal campione (pre-dichiarato in STATUS). ⛔ **CHIUSO 2026-08-11: FAIL 2/3** a n=20 → `--hedge` FALLITO, si torna alla v1 unhedged | giudice `hedged_vs_unhedged_judge.py` a n≥20 hedge-attivi: ① var ratio ≤0.6 ② drag medio ≤¼·SE ③ n≥20 → **① PASS (0.447, −55.3%) · ② FAIL (−0.647·SE vs −0.25) · ③ PASS** | **risposta: NO — l'hedge compra varianza (①) ma la paga**, e il costo è **76% fee perp / 0.9% funding**: B2 muore sul costo di realizzazione, non sulla teoria. Numeri: `TEORIA.md` §12.2 |
...
| C3 | ✅ **DONE 2026-07-18** — VPS service restarted with `--execute --hedge --hedge-band 0.30 --hedge-conv raw` (A4-frozen; NO ww) → **v2 forward STARTED**; partially-hedged open position EXCLUDED from the sample (pre-declared in STATUS). ⛔ **CLOSED 2026-08-11: FAIL 2/3** at n=20 → `--hedge` FAILED, back to the unhedged v1 | `hedged_vs_unhedged_judge.py` at n≥20 hedge-active: ① var ratio ≤0.6 ② mean drag ≤¼·SE ③ n≥20 → **① PASS (0.447, −55.3%) · ② FAIL (−0.647·SE vs −0.25) · ③ PASS** | **answer: NO — the hedge buys variance (①) but pays for it**, and the cost is **76% perp fees / 0.9% funding**: B2 dies on realization cost, not on theory. Numbers: `TEORIA.md` §12.2 |
...
🇮🇹
1. ✅ **FATTO 2026-07-30 — valutazione pre-registrata a n=30** (leg opzioni, §0.2), stessi 3 criteri e stessi 2 campioni: **FAIL 0/3 su entrambi** (primario 30 executed: PnL medio −0.00261 BTC, hit 0.400; NN −0.03480 batte always-LONG −0.07572 ma non always-SHORT **+0.03972**). I due campioni concordano → fail-safe sul peggiore non necessario; caveat pre-dichiarato: replay NN ricostruito n=28 < 30, ② "non valutabile" a rigore ma FAIL con margine ampio. Dettaglio in STATUS 2026-07-30. → **Sblocca A13/A14** (erano bloccati per non contaminare la leg opzioni): pre-registrazione **sizing v2** (A13 pin-close + A14 vega-sizing + A7 cablaggio greeks-risk, coda di rischio da **HAR-q90** — esito A2 definitivo).
...
4. **Derivazione `mfiv_30h`**: ✅ **FATTA** (D4 2026-07-18, incrementale periodica via `scripts/vol/derive_mfiv.py`; wedge MFIV−ATM stabile +3.39 vol pt su 1.911 tick). ⛔ **Promozione a comparatore dell'edge di `04b`: TENTATA e RESPINTA — gate v2 CHIUSO 2026-08-18, FAIL a `n=41` (`Δρ = +0.0343` < +0.05, segno incoerente fra le metà). Il comparatore live resta ATM IV, MFIV = colonna diagnostica permanente, item chiuso; il break-even short-vol è stato ri-stimato col wedge (MFIV +22.2% in varianza sull'ATM). Storico: pre-reg v2 PROMOSSA nella roadmap 2026-07-20** (subito dopo la chiusura A8-BIS, prima di B4-bis): break-even short-vol ri-stimato col wedge di convessità. Indipendente dall'esito del probe DVOL; dettaglio: memoria `idea_mfiv_30h`.
...
**EN**
1. ✅ **DONE 2026-07-30 — pre-registered n=30 evaluation** (options leg, §0.2), same 3 criteria and same 2 samples: **FAIL 0/3 on both** (primary, 30 executed: mean PnL −0.00261 BTC, hit 0.400; NN −0.03480 beats always-LONG −0.07572 but not always-SHORT **+0.03972**). The two samples agree → the worst-verdict fail-safe was not needed; pre-declared caveat: reconstructed NN replay n=28 < 30, so ② is strictly "not evaluable" but FAILs by a wide margin. Detail in STATUS 2026-07-30. → **Unblocks A13/A14** (frozen so far to avoid contaminating the options leg): **v2 sizing** pre-registration (A13 pin-close + A14 vega-sizing + A7 greeks-risk wiring, tail risk from **HAR-q90** — definitive A2 outcome).
...
4. **`mfiv_30h` derivation**: ✅ **DONE** (D4 2026-07-18, periodic incremental via `scripts/vol/derive_mfiv.py`; MFIV−ATM wedge stable at +3.39 vol pt over 1,911 ticks). ⛔ **Promotion to `04b` edge comparator: ATTEMPTED and REJECTED — v2 gate CLOSED 2026-08-18, FAIL at `n=41` (`Δρ = +0.0343` < +0.05, sign inconsistent across halves). The live comparator stays ATM IV, MFIV = permanent diagnostic column, item closed; the short-vol break-even was re-estimated with the wedge (MFIV +22.2% in variance over ATM). History: v2 pre-reg PROMOTED in the 2026-07-20 roadmap** (right after A8-BIS closes, ahead of B4-bis): short-vol break-even re-estimated with the convexity wedge. Independent of the DVOL probe outcome; detail: `idea_mfiv_30h` memory.
