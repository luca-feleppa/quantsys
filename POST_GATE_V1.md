...
| C4 | **VPS:** `--greeks` su `quantsys-iv.service` (+ daemon-reload) e cadenza poller 10→5 min | smoke: `atm_greeks.parquet` cresce, selezione = `pick_straddle` | rende la leg hedge v2 replayabile offline + valida la convenzione δ sul campo |
...
| C4 | **VPS:** `--greeks` on `quantsys-iv.service` (+ daemon-reload) and poller cadence 10→5 min | smoke: `atm_greeks.parquet` grows, selection = `pick_straddle` | makes the v2 hedge leg offline-replayable + field-validates the δ convention |
