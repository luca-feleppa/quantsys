...
| B4 | **RINVIATO 2026-07-19 → B4-bis** (mai girato, zero numeri visti): ri-derivare `lstm_dataset_dvol.npz` dal npz NUOVO (`dev_vols_dvol_append.py`) + NUOVA pre-reg col pattern-③ standard (STATUS 2026-07-20 ②) — solo DOPO la chiusura di A8-BIS | stesse condizioni su val | la IV risk-neutral (DVOL 30d) aggiunge contenuto predittivo oltre price+volume+macro? (prior onesto: nullo/sotto soglia per tenor mismatch 30d vs 30h) |
