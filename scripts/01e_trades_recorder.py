# Deribit option trades recorder (production, PUBLIC no-auth endpoint) —
...
# Deribit helpers shared with 01c (extracted 2026-07-16; production, no-auth:
...
# observed public endpoint retention (~24h): cold start rewinds this far;
...
# per-tick overlap with what's already saved — absorbs clock skew and
...
def fetch_trades_window(currency: str, start_ms: int, end_ms: int,
    ...
    # walks [start_ms, end_ms] forward (sorting=asc) paginating on has_more:
    ...
    for _ in range(max_pages):
        ...
        if not res.get("has_more") or last_ts >= end_ms:
            break
        # inclusive advance — if a whole page shares one ms (unrealistic for
    ...
    # column normalization: ts→UTC datetime + name-parsed fields.
...
def last_saved_ms(now: pd.Timestamp) -> int:
    # resumes from the last persisted trade (today's file, then yesterday's —
...
def poll_once(currency: str) -> dict:
    # one tick: [last saved − overlap, now] window → paginated fetch →
...
def main():
    ...
    while True:
        try:
            ...
            # transients (network, 5xx) don't kill the recorder: history
