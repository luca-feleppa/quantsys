# PAPER — DIRECTIONAL ECONOMETRIC BASELINES (pre-registered in STATUS.md
...
EPS = 1e-12
# expected counts on the current raw (65,191 candles, 2026-06-11 rs run). NB
...
def rebuild_split_timestamps(cfg: dict) -> dict:
    # rebuilds t_train/t_val/t_test replicating the 01 path: build with the
    ...
    # shared canonical derivation (C2 2ter).
    cols = canonical_feature_columns(fb.feature_cols, feat)
    log.info(f"canonico ricostruito: {len(cols)} feature")

    # create_windows replica without materializing X: window j covers rows
    ...
    max_idx = M - T - 1
    win_nan = csum[T:T + max_idx] - csum[0:max_idx]   # number of NaN rows in window j
    valid = win_nan == 0
...
def main():
    ...
    # raw target + trailing regressors from raw candles (same methodology as
    ...
    df = pd.DataFrame({
        ...
        "y":  lr.rolling(h).sum().shift(-h),                    # Σ log-ret over the next h bars
        "rh": lr.rolling(h).sum(),                               # trailing h bars
        "rw": lr.rolling(7 * bars_day).sum() * (h / (7 * bars_day)),    # 7d rescaled to h
        "rm": lr.rolling(30 * bars_day).sum() * (h / (30 * bars_day)),  # 30d rescaled to h
    ...
    # train-only fit: closed-form OLS + sign logit (same regressors).
    REG = ["rh", "rw", "rm"]
    ...
    for split in ("val", "test"):
        ...
        for name, p in preds.items():
            # Spearman undefined on the constant → 0 by convention (null).
