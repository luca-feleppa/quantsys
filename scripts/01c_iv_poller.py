# Deribit IV poller (2026-06-11 checklist, item 1) — FORWARD collection of BTC
...
# Deribit helpers shared with 01e (extracted 2026-07-16, ex local duplicates).
...
# ATM ~tenor straddle greeks (--greeks ONLY; 1 row/tick).
GREEKS_PATH = IV_DIR / "atm_greeks.parquet"

# target tenor in hours = the vol model's forecast_horizon (30 1h bars).
TENOR_HOURS = 30.0
# how many nearby expiries to keep in the ATM summary (short term structure).
N_EXPIRIES = 4
# DVOL availability start on Deribit (for the backfill).
DVOL_START = datetime(2021, 3, 24, tzinfo=timezone.utc)


def fetch_chain_snapshot(now: pd.Timestamp) -> pd.DataFrame:
    # 1 call → mark_iv of the whole BTC option chain; unparsable rows or
    ...
    for it in res:
        ...
        rows.append({
            ...
            "strike": strike,
            "option_type": opt,
            # mark_iv in PERCENT (Deribit convention, e.g. 45.2 = 45.2%).
...
def atm_term_structure(chain: pd.DataFrame, now: pd.Timestamp) -> pd.DataFrame:
    # for each of the N_EXPIRIES nearest live expiries: forward = per-expiry
...
def interp_iv_at_tenor(term: pd.DataFrame, tenor_hours: float) -> float:
    # LINEAR interpolation IN TOTAL VARIANCE w(T)=σ²·T (standard for IV term
...
def fetch_dvol(start_ms: int, end_ms: int, resolution: str = "3600") -> pd.DataFrame:
    # BTC DVOL index candles (max 1000 points/call); we keep the close.
...
def _finite_or_nan(v) -> float:
    # finite float or NaN (missing/null fields on illiquid strikes — 04b pattern).
    try:
        v = float(v)
...
def fetch_atm_greeks(chain: pd.DataFrame, now: pd.Timestamp) -> pd.DataFrame:
    # STATUS 2bis-① — venue greeks for the ATM straddle at the ~30h tenor.
...
def poll_once(greeks: bool = False) -> dict:
    # one full tick: raw chain → ATM term structure → IV@30h → DVOL
    ...
    # latest DVOL point (2h window to cover the in-progress hourly candle).
    ...
    # wide summary row: iv@30h + the N ATM expiries (tenor/iv/forward).
    ...
    # optional ATM greeks (--greeks): errors logged, NEVER propagated — the
...
def backfill_dvol() -> None:
    # hourly DVOL history backfill 2021-03-24→today: 1000 points/call,
...
def main():
    ...
    while True:
        try:
            ...
            # transients (network, 5xx) don't kill the poller: log and retry
