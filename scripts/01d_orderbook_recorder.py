# Binance L2 order-book recorder (roadmap track B1) — FORWARD collection of BTC
...
# shared collector atomic append (extracted 2026-07-16, ex local duplicate).
from quantsys.utils.collect import append_parquet             # noqa: E402

setup_logging()
log = logging.getLogger("quantsys.script.ob_recorder")

# Binance spot public endpoint (verified no-auth from this host, 2026-06-16).
BINANCE_DEPTH_URL = "https://api.binance.com/api/v3/depth"
OB_DIR = Path("data/orderbook")

# how many raw levels to persist per side (re-derivation safety net); the full
...
# bps bands around the mid for cumulative depth — a feature robust to level
# count (tick-size invariant, comparable over time).
DEPTH_BANDS_BPS = (5.0, 10.0, 25.0, 50.0)

# cross-tick state for Cont et al. OFI (needs the previous snapshot).
_PREV = {"bid_px": None, "bid_qty": None, "ask_px": None, "ask_qty": None}


def fetch_depth(symbol: str, levels: int) -> dict:
    # one REST snapshot of the book (best→worst, already sorted by Binance).
    r = requests.get(
        BINANCE_DEPTH_URL,
...
def _best_level_ofi(bid_px, bid_qty, ask_px, ask_qty) -> float:
    # best-level OFI (Cont-Kukanov-Stoikov) between two consecutive snapshots:
    ...
    if pb is None:
        return np.nan
    # bid side — a bid that rises or holds price adds buy pressure.
    if bid_px > pb:
        e_bid = bid_qty
    elif bid_px == pb:
        e_bid = bid_qty - qb
        ...
        e_bid = -qb
    # ask side — an ask that falls or holds adds sell pressure.
    if ask_px < pa:
        e_ask = ask_qty
    elif ask_px == pa:
        e_ask = ask_qty - qa
...
def compute_features(depth: dict, symbol: str) -> pd.DataFrame:
    # derive microstructure features from one snapshot. Everything is
    ...
    # microprice (Gatheral-Stoikov): mid weighted by opposite sizes — an
    ...
    # cumulative imbalance at increasing depths — a classic pressure sign.
    for n in (1, 5, 10, 20):
        sb = bid_qty[:n].sum()
        sa = ask_qty[:n].sum()
        feat[f"imbalance_L{n}"] = (sb - sa) / (sb + sa) if (sb + sa) > 0 else 0.0

    # cumulative depth within bps bands of the mid — invariant to level count.
    ...
    # top-N raw levels/side as list-columns — safety net: any future feature
...
def poll_once(symbol: str, levels: int) -> dict:
    # one full tick: REST snapshot → features → append to the day's parquet.
...
def main():
    # reconfigure UTF-8 (recurring cp1252 bug on Windows console — new-script checklist).
    ...
    # persistent loop — NOT a service: relaunch after a reboot (Start-Process,
    # see START.md). Network errors logged and non-fatal.
    n_ok, n_err = 0, 0
    while True:
        t0 = time.time()
        try:
            s = poll_once(args.symbol, args.levels)
            n_ok += 1
            if n_ok % 12 == 1:   # log ~every 1 min at 5s
                log.info(
        ...
        # sleep the remainder to keep cadence stable despite REST latency.
