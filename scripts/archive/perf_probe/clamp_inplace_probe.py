"""
Temporary probe — proves on REAL data that the pre-clip in 02_train.py
...
# ── Working-set measurement via Win32 (psutil is not installed in the venv) ──
# psapi PROCESS_MEMORY_COUNTERS: WorkingSetSize (current) and
...
# explicit argtypes/restype: the ctypes default (c_int) truncates the 64-bit
...
def _mem() -> tuple[int, int, int, int]:
    """
    (current WS, peak WS, current private commit, peak private commit) in bytes.
...
# ── Partial read of the npz member ───────────────────────────────────────────
def load_slice(n: int) -> np.ndarray:
    """
    reads the first n samples of X_train from the .npy member inside the zip
    ...
    with zipfile.ZipFile(NPZ) as z, z.open("X_train.npy") as f:
        ...
        # numpy 2.x no longer exposes _read_array_header — use the versioned reader.
        ...
        # readinto a preallocated array — a SINGLE allocation, so the load
...
def clip_bounds(x: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    """same bounds as 02_train.py (per-feature p0.1/p99.9)."""
...
def run_equal(n: int) -> int:
    ...
    # two INDEPENDENT copies of the same data (astype always copies, as in
    ...
    a_out = a.clamp(lo_t, hi_t)          # out-of-place (current variant)
    b_ptr = b.data_ptr()
    b.clamp_(lo_t, hi_t)                 # in-place (proposed variant)

    # exact count of actually clipped elements — without it the test would be
...
def run_peak(n: int, variant: str) -> int:
    # the Windows PeakWorkingSetSize CANNOT be reset (no API does it), so the
    ...
    # the Windows PEAK counters (PeakWorkingSetSize, PeakPagefileUsage) are
    ...
    if variant == "oop":
        t = t.clamp(lo_t, hi_t)          # allocates a new tensor
    elif variant == "inplace":
        t.clamp_(lo_t, hi_t)             # writes in existing storage
    else:
...
def _sampled(fn):
    """
    runs fn() while a thread samples the private commit — returns
