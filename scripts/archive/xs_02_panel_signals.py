"""
xs_02_panel_signals.py — PANEL-INFERENCE harness for the cross-sectional IC probe.

Applies the EXISTING model (heterogeneous ensemble, NO retraining) to each asset to
...
# Force UTF-8 on stdout/stderr — Windows cp1252 crashes on non-ASCII chars.
...
# Project root on sys.path so the quantsys/ package imports resolve.
...
# ── Pipeline constants (single source of truth = config + PipelineState) ────────
# forecast_horizon=30 (target = sum of next 30 log-returns); window=120×104.
# The step=30 (=h) grid yields NON-overlapping forward returns → quasi-independent bets.
ARCH_DIR = "itransformer"   # checkpoint/PipelineState dir
# 2026-05-16 — batch 256 keeps VRAM <3 GiB even with the Mamba branch on (see 03_backtest).
BATCH_SIZE = 256


# Builds a FeatureBuilder carrying BTC's scaler/clip/feature_cols (no fit). Exact
# replica of the replay path (99_replay_live_vs_training.py): the BTC transform is
# applied uniformly to the asset → keeps the cross-sectional ordering meaningful.
def _build_feature_builder(cfg: dict, ps: PipelineState) -> FeatureBuilder:
    ...
    # Inject BTC scaler state (multi-column + clip + legacy per-col).
    fb.scaler             = ps.scaler
...
# Normalize funding_df to open_time UTC ns-naive (FeatureBuilder._funding_features
...
# Module-level singletons for model/state/builder — loaded once and reused across all
...
# Loads (once) heterogeneous ensemble + BTC PipelineState + builder + canonical names.
...
def _ensure_loaded() -> None:
    ...
    # Canonical features (104) = single source of truth from the NPZ — training order.
    _CANONICAL = get_canonical_feature_names(str(ROOT / "data" / "lstm_dataset.npz"))

    # Heterogeneous ensemble (same loader as 03_backtest / 04_live).
...
# Inference for ONE symbol → LONG panel [open_time, mu_raw, sigma_raw, fwd_ret_30].
def generate_mu_for_symbol(raw_parquet_path: str | Path,
                           funding_parquet_path: str | Path) -> pd.DataFrame:
    """
    Applies the BTC ensemble to the asset's candles, emitting raw μ/σ on a grid every
    ...
    Args:
        raw_parquet_path:     the asset's OHLCV parquet (raw_candles schema: open_time,
                              open/high/low/close/volume/taker_buy_vol, ...).
        funding_parquet_path: the asset's funding rate parquet (open_time, funding_rate).
    ...
    # Grid step = horizon → NON-overlapping forward returns (quasi-independent).
    step = horizon

    # ── 1. Load the asset's raw candles ─────────────────────────────────────────
    raw_path = Path(raw_parquet_path)
    df_raw = pd.read_parquet(raw_path)
    # open_time → ordered UTC ns-naive. FeatureBuilder._funding_features reindexes the
    ...
    # Raw log-close-return series — basis for fwd_ret_30, aligned to the FULL timeline
    # (BEFORE FeatureBuilder's dropna, which trims the last h rows).
    raw_log_ret = np.log(df_raw["close"].to_numpy(dtype=np.float64))
    raw_log_ret = np.diff(raw_log_ret, prepend=raw_log_ret[0])   # ret[0]=0
    # fwd_ret_30[t] = sum of log-returns over (t, t+h] = ret[t+1] + ... + ret[t+h].
    # cumsum + shifted difference, NaN where <h future candles exist.
    csum = np.concatenate([[0.0], np.cumsum(raw_log_ret)])        # csum[k]=Σ ret[:k]
    n_raw = len(raw_log_ret)
    fwd_full = np.full(n_raw, np.nan, dtype=np.float64)
    valid_end = n_raw - horizon
    if valid_end > 0:
        idx = np.arange(valid_end)
        # Σ ret[idx+1 .. idx+h] = csum[idx+h+1] − csum[idx+1].
        fwd_full[idx] = csum[idx + horizon + 1] - csum[idx + 1]
    # Map open_time → forward return (robust lookup against feature-frame trims/gaps).
    fwd_by_time = pd.Series(fwd_full, index=df_raw["open_time"].to_numpy())

    # ── 2. Feature engineering with the BTC scaler (uniform transform, no fit) ──
    fd = _prepare_funding(funding_parquet_path)
    if fd is None:
        log.warning(f"{raw_path.name}: funding parquet assente — feature funding_* = 0.")
    # build(normalize=True, fit=False) → applies the injected BTC scaler (NO re-fit) +
    ...
    # Canonical feature matrix (fixed order) + timeline; drop residual-NaN rows (initial
    # rolling warmup) to avoid NaN windows in inference.
    feat_df = feat_df.copy()
    feat_df["open_time"] = pd.to_datetime(feat_df["open_time"])   # stays UTC ns-naive
    ...
    # ── 3. Decision grid every `step` candles ───────────────────────────────────
    # The window ending at row j uses feat_mat[j-window+1 : j+1]; first valid j=window-1.
    ...
    decision_js = np.arange(last_j, first_j - 1, -step)[::-1]    # chronological order
    if len(decision_js) == 0:
        return pd.DataFrame(columns=["open_time", "mu_raw", "sigma_raw", "fwd_ret_30"])

    # Stack windows (n_dec, window, n_feat) — stride-tricks view then copy.
    ...
    # ── 4. Batch inference (heterogeneous ensemble, AMP off internally) ─────────
    n_dec = len(windows)
    ...
    with torch.no_grad():
        for b0 in range(0, n_dec, BATCH_SIZE):
            ...
            # has_macro=False for these archs → forward without x_macro (the
    ...
    # ── 5. z-score → raw denormalization (CRITICAL INVARIANT) ───────────────────
    # Model predicts in z-score space; denormalize_predictions converts μ/σ to RAW space
    ...
    # Safety check — raw σ ≥ 5%/min is physically impossible: missed denorm or broken scaler.
    ...
    # ── 6. Align forward returns to the decision grid (lookup by open_time) ─────
...
# Main loop — iterates every data/xs/raw/{SYMBOL}.parquet, concatenates into a LONG panel.
...
def main() -> int:
    ...
    # open_time → datetime64[ms, UTC] (delivered schema).
