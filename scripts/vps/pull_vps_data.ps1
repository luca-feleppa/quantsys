# PULL DATA FROM THE COLLECTOR VPS (home side, Windows PowerShell 5.1+).
...
# project root = two levels above this script (scripts/vps/ -> root).
$ProjRoot = (Resolve-Path (Join-Path $PSScriptRoot "..\..")).Path

# shared secrets parser + anti-stall ssh options (common.ps1, refactor
...
# never print the host (private): only the time window.
Write-Output "[pull] finestra/window: $Days giorni/days"

# 0) PUSH home macro -> VPS, WITH DATED VINTAGES (rewritten 2026-07-31;
...
if (Test-Path $macroLocal) {
    # local vintage = last index date (helper: clean stdout).
    ...
    if ($LASTEXITCODE -ne 0 -or -not ($macroVintage -match '^\d{8}$')) {
        ...
        # scp to .tmp + remote mv: 04b must never read a half-written file.
        ...
        if ($LASTEXITCODE -ne 0) {
            ...
            # LIVE vintage = canonical symlink target. Empty = canonical is
            ...
            if ($liveVintage -eq $macroVintage) {
                ...
                # RELATIVE symlink (resolved from the link's dir) + mv -T =
...
# 1) append-only single files (small: re-copied whole every time).
...
# C4 (2026-07-18): --greeks poller ATM greeks - without this line the VPS
...
# 2) recent dailies - remote list via find -mtime, then per-file scp
...
# 2b) vol-paper (04b on the VPS since 2026-07-18): forward-test log + state
...
# 3) merge into the canonical copy + staleness heartbeat (unless -NoMerge).
