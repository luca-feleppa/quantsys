# VPS DATA MERGE → CANONICAL COPY (home side). Reconciles the files pulled
...
# tmp + os.replace — THEORY.md §12.5 safety net).
...
# project root (scripts/vps/ → 2 levels up); CWD-independent paths.
ROOT = Path(__file__).resolve().parents[2]
STAGING = ROOT / "data" / "vps_staging"

# merge map — (staging, canonical, dedup keys, time column for sorting).
SINGLE_FILES = [
    (STAGING / "iv" / "atm_30h.parquet", ROOT / "data" / "iv" / "atm_30h.parquet",
     ["timestamp"], "timestamp"),
    (STAGING / "iv" / "dvol.parquet", ROOT / "data" / "iv" / "dvol.parquet",
     ["timestamp"], "timestamp"),
    # C4 (2026-07-18) — ATM ~30h straddle greeks (01c --greeks): 1 row/tick.
...
DAILY_DIRS = [
    ...
    # Deribit option trades (01e): trade_id is the venue's natural key.
...
def merge_one(staged: Path, canon: Path, keys: list[str], ts_col: str) -> tuple[int, int]:
    # merges ONE staged file into the canonical one: pure copy if the
...
# ── vol-paper (04b on the VPS since 2026-07-18) ──────────────────────────
...
def merge_jsonl(staged: Path, canon: Path, keys: tuple) -> tuple[int, int]:
    # key-based union (canonical first, staging wins on duplicates), sorted
...
def merge_vol_paper():
    if not VP_STAGING.exists():
        return
    # forecasts: dedup on candle_ts, VPS rows winning (see section header).
    ...
    # presence mirror ONLY after a successful pull (marker): copy or delete.
...
def heartbeat(stale_hours: float) -> bool:
    # VPS staleness measured on the STAGING files (they snapshot the remote
    # collector state at pull time). True = everything fresh.
    ok = True
    now = pd.Timestamp.now(tz="UTC")
    # per-check threshold: 01e trades are written only when the market
    ...
    if tr_files:
        checks.append(("Trades recorder (deribit_trades)", tr_files[-1], "timestamp",
                       max(stale_hours, 6.0)))
    # 04b (VPS since 2026-07-18): candle_ts is the tick's CLOSED candle →
...
def main() -> int:
    ...
    # 1) append-only single files.  2) per-directory dailies.
    ...
    # 2026-07-18 hygiene — staging duplicates ~40 MB already merged into the
