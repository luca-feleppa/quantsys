...
# project root on sys.path so the shared modules import without an editable install.
_ROOT = Path(__file__).resolve().parents[1]
if str(_ROOT) not in sys.path:
    sys.path.insert(0, str(_ROOT))
from quantsys.data.deribit import parse_instrument  # noqa: E402
...
def _parse_instrument(name: str):
    # single parser shared with the collectors (quantsys/data/deribit.py).
    return parse_instrument(name or "")
...
LEDGER_PATH = Path("results/vol_paper/hedge_ledger.jsonl")
FUNDING_CACHE = Path("data/deribit_funding_perp.parquet")
HEDGE_JUDGE_PATH = _ROOT / "scripts" / "vol" / "hedged_vs_unhedged_judge.py"


def _trade_row(t: dict, now_ms: float) -> dict:
    # normalize a 04b record: straddle (v1), iron butterfly (--adaptive), pin-close
    # exit, or an INCOMPLETE structured attempt (no position, no PnL). The open
    # position (position.json) has the same schema minus the settlement fields.
    structure = t.get("structure") or "straddle"
    incomplete = t.get("exit_mode") == "incomplete"
    wings = t.get("wing_strikes")
    prem_body = float(t.get("prem_call", 0) or 0) + float(t.get("prem_put", 0) or 0)
    prem_wings = float(t.get("prem_wing_call", 0) or 0) + float(t.get("prem_wing_put", 0) or 0)
    settled = t.get("settled_ts") is not None and not incomplete
    expiry_ms = _optf(t.get("expiry_ms"))
    return {
        "entry_ts": t.get("entry_ts"), "settled_ts": t.get("settled_ts"),
        "structure": structure, "exit_mode": t.get("exit_mode"),
        "incomplete": incomplete, "recovery": t.get("recovery"),
        "side": int(t.get("side", -1 if incomplete else 1)),  # 1 LONG, -1 SHORT
        "executed": bool(t.get("executed", False)),
        "settled": settled,
        # expiry passed but no settlement record yet (04b not settled or not pulled).
        "expired_pending": (not settled and not incomplete and expiry_ms is not None
                            and expiry_ms <= now_ms),
        "strike": _optf(t.get("strike")),
        "wing_strikes": wings if isinstance(wings, list) else None,
        "entry_spot": _optf(t.get("index_at_entry")),
        "delivery_price": _optf(t.get("delivery_price")),
        "index_at_exit": _optf(t.get("index_at_exit")),
        "prem_call": _safe(t.get("prem_call")), "prem_put": _safe(t.get("prem_put")),
        # net premium per contract: body minus the wings paid (0 wings for a straddle).
        "premium": _safe(prem_body - prem_wings), "premium_wings": _safe(prem_wings),
        "fee_btc": _safe(t.get("fee_btc")), "exit_fee_btc": _optf(t.get("exit_fee_btc")),
        "amount": _safe(t.get("amount", 1.0)),
        "payoff_btc": _optf(t.get("payoff_btc")), "pnl_btc": _optf(t.get("pnl_btc")),
        "edge": _optf(t.get("edge")), "rv_pred": _optf(t.get("rv_pred")),
        "var_iv": _optf(t.get("var_iv")), "t_hours": _optf(t.get("t_hours_at_entry")),
        "expiry_ms": expiry_ms,
...
def _hedge_tools():
    # perp-leg reconstruction reused from the hedged-vs-unhedged judge (exact inverse
    # PnL, ledger fees, accrued funding): one formula, not a copy. Funding comes from
    # the local cache only (no network from the terminal); memoized by file mtimes.
    key = tuple(p.stat().st_mtime if p.exists() else None
                for p in (LEDGER_PATH, FUNDING_CACHE, HEDGE_JUDGE_PATH))
    if _HEDGE_CACHE.get("key") == key:
        return _HEDGE_CACHE["val"]
    val = None
    if LEDGER_PATH.exists() and HEDGE_JUDGE_PATH.exists():
        try:
            import importlib.util as _ilu
            spec = _ilu.spec_from_file_location("hedged_vs_unhedged_judge", HEDGE_JUDGE_PATH)
            judge = _ilu.module_from_spec(spec)
            spec.loader.exec_module(judge)
            by_key: dict = {}
            for e in judge._read_jsonl(LEDGER_PATH):
                if e.get("position_key"):
                    by_key.setdefault(judge._pos_key(e["position_key"]), []).append(e)
            funding = (pd.read_parquet(FUNDING_CACHE) if FUNDING_CACHE.exists()
                       else pd.DataFrame({"timestamp": pd.Series(dtype="int64")}))
            f_end = int(funding["timestamp"].max()) if len(funding) else None
            val = {"judge": judge, "by_key": by_key, "funding": funding, "funding_end_ms": f_end}
        except Exception as e:
            log.warning(f"hedge ledger not readable, perp leg omitted: {type(e).__name__}: {e}")
    _HEDGE_CACHE.update(key=key, val=val)
    return val


def _attach_hedge(row: dict, tools) -> None:
    # perp PnL of the hedge leg for a settled position: gross - fees - funding.
    # Settled positions without ledger events get 0 (never hedged); open ones stay None.
    row.update(hedge_pnl_btc=None, hedge_events=0, hedge_flags=[],
               pnl_total_btc=row["pnl_btc"])
    if tools is None or row["strike"] is None or row["expiry_ms"] is None:
        return
    events = tools["by_key"].get((row["side"], float(row["strike"]), int(row["expiry_ms"])), [])
    row["hedge_events"] = len(events)
    if not row["settled"] or row["pnl_btc"] is None:
        return
    if not events:
        row["hedge_pnl_btc"] = 0.0
        return
    leg = tools["judge"].perp_leg(events, tools["funding"], int(row["expiry_ms"]))
    net = leg["pnl_perp_gross"] - leg["fees_perp"] - leg["funding_paid"]
    flags = [f for f in ("has_reconcile", "open_residual") if leg[f]]
    last_ms = max(pd.Timestamp(e["ts"]).value // 10**6 for e in events)
    if tools["funding_end_ms"] is None or tools["funding_end_ms"] < min(last_ms, row["expiry_ms"]):
        flags.append("funding_cache_short")
    row.update(hedge_pnl_btc=_safe(net), hedge_flags=flags,
               pnl_total_btc=_safe(row["pnl_btc"] + net))
    ...
    if TRADES_PATH.exists():
        for line in TRADES_PATH.read_text(encoding="utf-8").strip().splitlines():
            if not line.strip():
                continue
            try:
                rows.append(_trade_row(json.loads(line), now_ms))
            except Exception:
                continue
    # open position appended last (always the most recent).
    n_open = 0
    if POSITION_PATH.exists():
        try:
            pos = json.loads(POSITION_PATH.read_text(encoding="utf-8"))
            if isinstance(pos, dict) and pos.get("strike") is not None:
                rows.append(_trade_row(pos, now_ms))
                n_open = 1
        except Exception:
            log.warning("position.json illeggibile — riga open omessa / unreadable, open row skipped")
    if not rows:
        return {"trades": [], "summary": {"n": 0, "n_settled": 0, "n_open": 0,
                                          "note": "no trades (04b forward test not started)"}}
    tools = _hedge_tools()
    for r in rows:
        _attach_hedge(r, tools)
    settled = [r for r in rows if r["settled"] and r["pnl_total_btc"] is not None]
    pnls = [r["pnl_total_btc"] for r in settled]
    n_s = len(settled)
    wins = sum(1 for p in pnls if p > 0)
    summary = {
        "n": len(rows), "n_settled": n_s, "n_open": n_open,
        "n_executed": sum(1 for r in rows if r["executed"]),
        "n_incomplete": sum(1 for r in rows if r["incomplete"]),
        "n_expired_pending": sum(1 for r in rows if r["expired_pending"]),
        "n_hedged": sum(1 for r in settled if r["hedge_events"]),
        # totals: option legs, perp hedge leg, and their sum (the stats below use the sum).
        "options_pnl": _safe(sum(r["pnl_btc"] for r in settled)) if settled else 0.0,
        "hedge_pnl": _safe(sum(r["hedge_pnl_btc"] or 0.0 for r in settled)) if settled else 0.0,
        "total_pnl": _safe(sum(pnls)) if pnls else 0.0,
        "hedge_ledger": tools is not None,
...
<div id="page-trades" class="page">
  ...
  <div class="grid2">
    <div class="panel">
      <h3>Trade History <span style="font-weight:400;text-transform:none;letter-spacing:0">04b short-vol forward test (Deribit testnet)</span></h3>
      <div class="scroll">
        <table id="trades-table"><thead><tr id="trades-head"></tr></thead><tbody id="trades-body"></tbody></table>
      </div>
      <div class="legend">Click a row for its risk profile. LONG = long vol (profits if |move| &gt; breakeven), SHORT = short vol. Premium, payoff and PnL in BTC. Hedge PnL = perp delta-hedge leg (exact inverse PnL − fees − funding from the local cache; * = flagged, hover the row); — = no perp fill on that position. Status: settled · pin-close · open · expired, awaiting settlement (expiry passed, no settlement record on disk yet) · simulated (fill at mark, not executed) · incomplete (structured entry not completed).</div>
    </div>
    <div class="panel">
      ...
      <div class="legend">Option-leg PnL (BTC) at expiry vs underlying. ◆ = actual settlement. Lines: strike (dotted) · entry spot (amber) · breakevens (dashed green). Inverse Deribit options: PnL = side·(|S−K|/S − premium) − fee; an iron butterfly subtracts the long wings' payoff. The perp hedge is path-dependent and is reported in the title, not in the curve.</div>
    </div>
  </div>
</div>
...
// 2D config — no box "window" zoom (dragmode 'pan'), but axis zoom via wheel
...
// ── plot() — SINGLE render guard (holistic). The "bars/curves vanish on tab switch"
...
function plot(id, traces, layout, cfg, _t){
  ...
  if(!el) return;
  // (1) real-screen size-guard: getBoundingClientRect reflects visibility/transform.
  const rc = el.getBoundingClientRect();
  if(rc.width < 2 || rc.height < 2){
    if((_t||0)<60) requestAnimationFrame(()=>plot(id,traces,layout,cfg,(_t||0)+1));
    return;
  }
  // (2) full rebuild on tab re-entry (clip-paths to redo) or a never-drawn div;
...
// null-safe percentage: '—' without the % suffix (backend sends null when data is missing).
function fmtPct(v,d=1){ return (v==null||isNaN(v)) ? '—' : fmt(v,d)+'%'; }
function cls(v){ return v>=0?'pos':'neg'; }
// explicit-SIGN formatter (+ for positives; negatives already carry −) for the greeks.
...
// full greek names for headers/labels
...
function switchTab(name){
  ...
  // stamp the tab-switch instant: for the next ~4s plot() does a newPlot (rebuild the
  // clip-paths corrupted by display:none→block), then reverts to react.
  window.__tabShownAt = Date.now();
  // the page is NOW display:block; loaders are async but every render goes through
...
async function ensureExpiries(){
  // 10-min TTL — dailies expire at 08:00 UTC and new ones get listed: without a
...
// FRACTIONAL position of a value on a CATEGORY axis (categories = ascending arr).
...
// ─── Risk & Greeks ────────────────────────────────────────────────────────────
// per-strike OI profile — DIVERGING bars (calls ▲ green up, puts ▼ red down),
...
function renderOI(d){
  ...
  // FILTER data to the spot±35% band BEFORE plotting (not just the view): X and Y
  ...
  // sort the band by ASCENDING strike → x-axis categories follow strike order (a
  ...
  // ── CATEGORY x-axis (not linear) = THE fix for the long-standing "bars vanish /
  ...
  plot('plot-oi', [
    ...
    // Net OI = call−put → same unit/scale as the bars → PRIMARY axis (no yaxis2).
    ...
    hovermode:'x unified',
    // category axis + thinned ticks (~1 every n/9) to avoid overlapping 40+ strikes.
...
async function loadRisk(){
  try{
    ...
    // async↔tab race GUARD. loadRisk is async: between fetch start and resolve the
    ...
    // data guard — if the response is degraded (partial/desynced chain: no
    ...
    if(_noStrk || _noOI){ console.warn('risk: partial/out-of-sync data → skip redraw'); return; }
...
let TRADES = [];
let SEL_TS = null;   // selected trade entry_ts (survives the 12s refresh)
async function loadTrades(){
  try{
    const d = await (await fetch('/api/trades')).json();
    TRADES = d.trades||[];
    const s = d.summary||{};
    const hedgeSub = s.hedge_ledger
      ? `options ${fmtS(s.options_pnl,4)} · perp hedge ${fmtS(s.hedge_pnl,4)} (${s.n_hedged||0} hedged)`
      : 'options only (no hedge ledger on disk)';
    document.getElementById('trades-cards').innerHTML = [
      {l:'Total PnL (BTC)', v:fmtS(s.total_pnl,4), s:hedgeSub, c:cls(s.total_pnl||0)},
      {l:'Hit-rate', v:(s.hit_rate==null?'—':fmt(s.hit_rate*100,0)+'%'), s:`total PnL > 0 · ${s.n_settled||0} closed`, c:(s.hit_rate==null?'amb':(s.hit_rate>=0.5?'pos':'neg'))},
      {l:'Avg PnL/trade', v:(s.avg_pnl==null?'—':fmtS(s.avg_pnl,4)), s:'BTC, options + hedge', c:cls(s.avg_pnl||0)},
      {l:'Trades', v:fmt0(s.n||0), s:`${s.n_executed||0} executed · ${s.n_open||0} open`
          + (s.n_expired_pending ? ` · ${s.n_expired_pending} expired, awaiting settlement` : '')
          + (s.n_incomplete ? ` · ${s.n_incomplete} incomplete` : ''),
        c:(s.n_expired_pending||s.n_incomplete)?'neg':'amb'},
      {l:'Best / Worst', v:(s.best==null?'—':fmtS(s.best,4))+' / '+(s.worst==null?'—':fmtS(s.worst,4)), s:'BTC, options + hedge', c:'amb'},
    ].map(c=>`<div class="card"><div class="lbl">${c.l}</div><div class="val ${c.c||''}">${c.v}</div><div class="sub">${c.s}</div></div>`).join('');

    const cols=['#','Entry','Structure','Side','Strike','Entry spot','Net premium','Settle','Options PnL','Hedge PnL','Total PnL','Edge','Status'];
    ...
    document.getElementById('trades-body').innerHTML = TRADES.map((t,i)=>{
      ...
      const strike = t.strike==null ? '—'
        : fmt0(t.strike) + (t.wing_strikes ? ` [${t.wing_strikes.map(fmt0).join(' / ')}]` : '');
      const hedge = (t.hedge_pnl_btc==null || !t.hedge_events) ? '—'
        : fmtS(t.hedge_pnl_btc,4) + ((t.hedge_flags||[]).length ? ' *' : '');
      return `<tr onclick="selectTrade(${i})" style="cursor:pointer" title="${(t.hedge_flags||[]).join(', ')}">
        <td>${i+1}</td><td>${(t.entry_ts||'').slice(5,16)}</td>
        <td>${t.structure==='iron_butterfly'?'iron fly':t.structure}</td>
        <td class="${t.side>0?'pos':'neg'}">${t.side>0?'LONG':'SHORT'}</td>
        <td>${strike}</td><td>${fmt0(t.entry_spot)}</td>
        <td>${t.incomplete?'—':fmt(t.premium,4)}</td>
        <td>${t.delivery_price!=null?fmt0(t.delivery_price):(t.index_at_exit!=null?fmt0(t.index_at_exit):'—')}</td>
        <td class="${cls(t.pnl_btc||0)}">${fmtS(t.pnl_btc,4)}</td>
        <td class="${t.hedge_events?cls(t.hedge_pnl_btc||0):''}">${hedge}</td>
        <td class="${cls(t.pnl_total_btc||0)}">${fmtS(t.pnl_total_btc,4)}</td>
        <td>${fmt(t.edge,2)}</td><td class="${stat.c}">${stat.l}</td></tr>`;
    }).join('');
    // restore the user's selection by entry_ts (the 12s refresh rebuilds the table:
    ...
    else { document.getElementById('payoff-title').textContent='(no trades)'; Plotly.purge('plot-payoff'); }
    setConn(true);
  }catch(e){ setConn(false); }
}
// row status: incomplete attempt, not executed (simulated fill), open, expired but not
// yet settled (04b not settled or data not pulled), closed early, settled.
function tradeStatus(t){
  if(t.incomplete) return {l:`incomplete (${t.recovery||'?'})`, c:'neg'};
  if(t.expired_pending) return {l:'expired, awaiting settlement', c:'neg'};
  if(!t.settled) return {l:'open', c:'amb'};
  if(!t.executed) return {l:'simulated', c:''};
  if(t.exit_mode==='pin_close') return {l:'pin-close', c:''};
  return {l:'settled', c:''};
}
...
// risk profile at expiry — PnL (BTC) of the OPTION legs vs underlying, with the SAME
// settlement formula as 04b (maybe_settle), inverse Deribit options:
//   straddle:       pnl(S) = side·amt·(|S−K|/S − premium) − fee
//   iron butterfly: the long wings add −side·amt·(max(S−Kc,0)+max(Kp−S,0))/S, and
//                   `premium` is already net of the wings paid.
// → the settlement ◆ marker lands EXACTLY on the curve. Breakevens are the zero
// crossings of the curve on the grid (linear interpolation), valid for both shapes.
// The perp hedge PnL is path-dependent, not a function of S at expiry: it is shown
// in the title, never folded into the curve.
function renderPayoff(t){
  if(t.incomplete || t.strike==null){
    document.getElementById('payoff-title').textContent =
      `${t.structure} attempt INCOMPLETE · recovery ${t.recovery||'?'} · no position`;
    Plotly.purge('plot-payoff');
    return;
  }
  const K=t.strike, amt=t.amount||1, side=t.side>0?1:-1;
  const prem=t.premium||0, fee=t.fee_btc||0;
  const wings=t.wing_strikes;   // [Kc, Kp] for an iron butterfly, else null
  const pnlAt = S => {
    let pay = Math.abs(S-K)/S;
    if(wings) pay -= (Math.max(S-wings[0],0) + Math.max(wings[1]-S,0))/S;
    return side*amt*(pay - prem) - fee;
  };
  // K±20% window (wider for the wings), EXTENDED to include the settlement when it
  // falls outside (audit MINOR-5: an off-range delivery used to snap to the edge).
  const dp = (t.delivery_price!=null && t.delivery_price>0) ? t.delivery_price : null;
  let lo=K*0.80, hi=K*1.20;
  if(wings){ lo=Math.min(lo, wings[1]*0.95); hi=Math.max(hi, wings[0]*1.05); }
  ...
  const bes=[];
  for(let j=1;j<N;j++){
    if((ys[j-1]<0) !== (ys[j]<0)){ bes.push(xs[j-1] + (xs[j]-xs[j-1])*(-ys[j-1])/((ys[j]-ys[j-1])||1)); }
  }
  const beDn = bes.length ? bes[0] : null, beUp = bes.length>1 ? bes[bes.length-1] : null;
  // ── CATEGORY x-axis here too (SAME fix as renderOI). Cause (browser diagnosis
  ...
  // settlement marker only on closed trades with a valid delivery (>0: the
  ...
  // debit (long: premium+fee) / credit (short: premium−fee) + breakevens + hedge in title.
  const netCash = prem*amt + side*fee;
  const state = t.incomplete ? '' : t.expired_pending ? ' · EXPIRED, awaiting settlement'
    : !t.settled ? ' · OPEN' : (t.exit_mode==='pin_close' ? ' · closed early (pin-close)' : '');
  document.getElementById('payoff-title').textContent =
    `${t.side>0?'LONG':'SHORT'} ${t.structure==='iron_butterfly'?'iron butterfly':'straddle'} · K ${fmt0(K)}`
    + (wings ? ` · wings ${fmt0(wings[1])} / ${fmt0(wings[0])}` : '')
    + ` · ${t.side>0?'debit':'credit'} ${fmt(netCash,4)} BTC`
    + (beDn&&beUp ? ` · BE ${fmtK(beDn)} / ${fmtK(beUp)}` : '')
    + (t.hedge_pnl_btc ? ` · perp hedge ${fmtS(t.hedge_pnl_btc,4)} BTC` : '')
    + state;
  // render via plot() (size-guard + rebuild on re-entry); category x-axis (see above).
  plot('plot-payoff', traces, Object.assign({}, PL_DARK, {
    ...
    shapes:[
      ...
      // dashed breakevens (PnL=0), only when inside the plotted window.
    ...
    // no 'BE' labels (they collide with ticks below and the V inside): values
    ...
    annotations:[
      ...
      {x:_eX,y:0.97,yref:'paper',text:'Entry',showarrow:false,font:{color:'#f0a020',size:10},yanchor:'top',xanchor:'right'},
    ],
  }), PL_CFG_2D);
}
