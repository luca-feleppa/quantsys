# SEMIVARIANCE PROBE — HAR-RS JUDGE (pre-registered in STATUS.md 2026-06-11).
...
EPS = 1e-12  # same ε as the FeatureBuilder target
H = 30       # horizon fallback (effective from config)


# metrics for one prediction set vs truth — MSE (primary), Spearman, sign-DA.
...
def main():
    ...
    # ── Ground truth + HAR-RS components from raw candles (same definition as the target) ──
    ...
    # trailing log-ratio over h / 7d / 30d windows (scale-free: window length cancels
    ...
    y_fwd = (np.log(sq_pos.rolling(h).sum().shift(-h) + EPS)
             - np.log(sq_neg.rolling(h).sum().shift(-h) + EPS))  # same formula as the FeatureBuilder
    ...
    cols = ["xh", "xw", "xm", "xrv"]
    # ── Baseline 1: HAR-RS (closed-form OLS, fit on train) ──────────────────────
    ...
    # ── NN: ensemble forward on X_{split} → z → raw log-ratio (center+scale) ────
    ...
    # sanity: the log-ratio is near-centered (|c|<2) — a center ≈ −7 would flag
    ...
    # FULL z→raw inversion: μ·IQR + center.
