# DVOL-AS-FEATURE PROBE (STATUS pre-reg 2026-07-17) — derives
...
# pre-registered constants (STATUS 2026-07-17) — no sweep.
...
def build_dvol_features(timestamps: pd.DatetimeIndex, dvol: pd.DataFrame,
    ...
    # builds the 3 RAW features (pre-normalization) on the given timestamps.
    ...
    # row order MUST match X_{split} → monotonic input required.
    ...
    # datetime resolution unified to ns — the npz is [ms], the parquet [us]:
    ...
    if fill is None:
        # medians on the available portion of THIS call only (train).
        if not avail.any():
    ...
    # deliberate .to_numpy() — avoids pandas index re-alignment
...
def main() -> None:
    ...
    # production npz snapshot for the final untouched assert.
    ...
    # raw features per split — fill medians from the TRAIN split only (no leakage).
    raw, fill = {}, None
    ...
    # normalization of the 2 continuous columns via the existing macro
    ...
    # deliberately uncompressed: 5-10× faster, float32 barely compresses.
    atomic_save_npz(DST_NPZ, compressed=False, **arrays)
    log.info(f"Salvato {DST_NPZ} ({DST_NPZ.stat().st_size / 1e9:.2f} GB)")

    # post-save VERIFY — key-by-key bit-identity vs production (lazy: one
