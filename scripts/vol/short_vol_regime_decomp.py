...
The historical backtest (`short_vol_hist_backtest.py`) shows the short-vol edge exists ON AVERAGE over
2019-26. Critical question before promoting the arm (classic "picking up pennies in front of a
steamroller"): does the edge HOLD across all regimes or COLLAPSE in Stress (where it runs you over)?
Here each expiry is tagged with the dominant regime at entry (causal, from data/regime_probs.parquet —
R0 Quiet / R1 Trending / R2 Stress) and decomposed:
  • PnL by REGIME (mean/hit/worst): if the tail is all in Stress but the sign holds → robust;
    if Stress flips negative → a regime FILTER is needed.
  • PnL by YEAR (which years carried the edge; did 2022 break it?).
  • EQUITY curve + MAX DRAWDOWN of the strategy as a portfolio (DD depth/recovery).
  • REGIME-GATED variant (short only Quiet+Trending, flat in Stress) vs always-short.

Realistic NET-of-bid PnL: premium = FHS fair-value × (1+VRP) × (1−bid_haircut), per-structure
haircut from the validation (ATM 3.5%, strangle ~16%). VRP=0 ≈ "sell at mark" (see validation).

REGIME-DEPENDENT HAIRCUT (FIX ③, 2026-06-26). The base haircut comes from n=3 observations of ONE
...
⚠ LOAD-BEARING n=3 CAVEAT. Both the haircut LEVELS and the "VRP=0 ≈ sell at mark" equivalence rest
...
Usage:  python scripts/vol/short_vol_regime_decomp.py [--vrp 0.0] [--stress-haircut-mult 2.5]
"""
import sys
import json
import argparse
...
# BASE per-struct bid haircut (FHS-vs-mark validation, n=3 LOAD-BEARING on a calm 12-day window →
...
def pnl_series(tr, vrp, haircut):
    # per-trade NET-of-bid PnL (BTC). premium received = fair_value·(1+VRP)·(1−haircut);
    # fee on 2 legs ≈ premium/2 each (12.5% cap). PnL = premium − real_payoff − fee.
    # `haircut` may be SCALAR or a PER-TRADE vector (FIX ③: regime-dependent haircut) →
    # numpy broadcasts (1−haircut) row-wise.
    prem = tr["fair_value"].to_numpy() * (1.0 + vrp) * (1.0 - np.asarray(haircut, dtype=float))
    # 2-leg fee ≈ premium/2 (12.5% cap), vectorized
...
def regime_haircut_vector(m, base_hc, stress_mult):
    # FIX ③. PER-TRADE haircut vector = base × (stress_mult if regime==2 Stress, else 1.0). n/d-regime
    ...
    stress = (reg == 2.0)                                # NaN==2 → False (NaN→base)
    return base_hc * np.where(stress, float(stress_mult), 1.0)


def tag_regime(tr):
    # dominant regime at entry (backward merge_asof = regime ≤ entry only, CAUSAL). Excludes burn-in
    # (unreliable regime). regime_probs ends 2026-06-10 → later trades = NaN.
    ...
    # align datetime resolution (regime=ms, entry=us) → ns, else merge_asof raises.
...
def stats(pnl, ann_factor):
    # PnL vector stats. Annualized Sharpe = mean/sd × ann_factor, ann_factor = sqrt(trades_per_year)
    ...
    eq = np.cumsum(pnl)
    dd = eq - np.maximum.accumulate(eq)               # drawdown vs running peak
    maxdd = float(dd.min())
    tot = float(pnl.sum())
...
def ann_factor_from_trades(m):
    # Sharpe-coherence fix. trades/year from the REAL entry-timestamp span (DAILY cadence) →
    # ann_factor = sqrt(trades_per_year); fallback to the legacy factor sqrt(ANNUAL_BARS/TENOR_H) if
    # the span is null (1 trade).
...
def main():
    ...
    # FIX ③. Bid-haircut multiplier on Stress-regime (R2) trades ONLY: real bid-ask spread blows up
    ...
    # load-bearing n=3 caveat printed at run start (small-sample anchor holding the whole PnL thesis).
    ...
    report = {"meta": {"vrp": args.vrp, "bid_haircut": BID_HAIRCUT,
              ...
              "configs": [], "sensitivity": []}
    # causal GARCH series computed ONCE, reused by the 3 configs (A1). Fresh per-config rng
    ...
    for struct, w in configs:
        ...
        # FIX ③. FIRST tag the regime (causal), THEN compute the PER-TRADE haircut (Stress×mult) and
        ...
        # kernel-consistent ann_factor (sqrt(trades/year) from the real span), reused by every stats().
        ann_factor, tpy = ann_factor_from_trades(m)
        overall = stats(m["pnl"].to_numpy(), ann_factor)
        # block-bootstrap CI (overlap/clustering-robust) of the OVERALL on time-ordered PnL (m is
        ...
        # honest overall CI (5–95%): a mean-CI spanning 0 ⇒ edge indistinguishable from noise.
        ...
        # ── REGIME-GATED variant: flat in Stress (R2)
        gated = m[m["regime_dominant"] != 2]            # excludes Stress (NaN included = prudent skip)
        ...
        # ── SENSITIVITY of the two headlines to the widened Stress haircut (FIX ③)
        # (a) is Stress still the best edge? = Stress mean > 0 AND ≥ Quiet/Trending mean.
