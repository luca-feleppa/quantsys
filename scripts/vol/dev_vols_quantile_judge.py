# A2a — log-RV QUANTILE CALIBRATION JUDGE (pre-registered in STATUS.md 2026-07-08).
# The PASS vol model is already loss_type=quantile (QUANTILES=[.1,.25,.5,.75,.9]): this judge EXTRACTS the
...
# pinball loss for a single level τ, in log-RV space (mean over samples).
...
def main():
    ...
    # A2-CONFORMAL (pre-registered STATUS.md 2026-07-10) — inert by default:
    ...
    # ── Ground truth + HAR regressors (SAME data path as the QLIKE judge) ───────
    # identical definitions to dev_vols_qlike.py — one single way to compute y.
    ...
    # ── HAR-quantile baseline: OLS point (train fit) + empirical residual quantiles ──
    ...
    har_point = Xev @ beta
    # HAR q_τ = point + empirical train-residual quantile (standard QR baseline).
    har_q = {tau: har_point + float(np.quantile(resid_tr, tau)) for tau in QUANTILES}

    # ── NN: per-member quantiles → weighted Vincentization → z→raw inversion ────
    ...
    # every member MUST be quantile-type (this judge extracts the quantile head).
    members = model._models
    for m in members:
        assert getattr(m, "loss_type", "") == "quantile", \
            f"membro non-quantile ({getattr(m, 'loss_type', '?')}) — A2a non applicabile"
    w = np.asarray(model.weights, dtype=np.float64)  # current weights
    ...
    qp_members = []  # per-member (n_obs, Q) list
    with torch.no_grad():
        for mi, m in enumerate(members):
            ...
            for i in range(0, len(X), 256):
                ...
                qp, _ = out[0].sort(dim=-1)   # sort = non-crossing
    ...
    # Vincentization = weighted cross-member quantile average (level by level).
    qp_ens_z = np.tensordot(w, qp_all, axes=(0, 0))             # (n_obs, Q)
    # monotone affine z→raw log-RV inversion: quantiles transform directly.
    qp_ens = qp_ens_z * s + c
    ...
    if args.conformal:
        # ── A2-CONFORMAL: temporal prefix/suffix split of ev (already in ────────────
        #    chronological order: har.index is monotone and sel preserves it) ─────
        # per-level additive shift δ_τ = quantile_τ(y − q_τ) fit on the prefix
        ...
        # NON-decisional diagnostic: q90−q10 width pre/post on the suffix.
        conf["width_q10_q90"] = {
    ...
    # ── Metrics: coverage + pinball (NN vs HAR-quantile) ────────────────────────
