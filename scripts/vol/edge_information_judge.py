# E1 JUDGE — DOES THE NN-vs-IV EDGE CARRY PREDICTIVE CONTENT ABOUT REALIZED
...
# ⚠ CONSTANTS HARDCODED BECAUSE PRE-REGISTERED. Changing them after seeing
# results is goalpost-moving: the pre-reg forbids any variant without a NEW
# registration.
# ⚠ TWO STAGES. Stage 1 = EXPLORATORY on the already-recorded window: no
# thresholds, NO VERDICT, hypothesis-generating only (the sample carries partial
# information about the outcome, because the PnL of those expiries already failed
# the 30/07 gate). Stage 2 = CONFIRMATORY on expiries settling AFTER the
...
H_BARS = 30                 # target horizon and var_iv tenor
DECISION_LO, DECISION_HI = 27.0, 33.0   # decision-tick window (hours before expiry)
EXPIRY_HOUR_UTC = 8         # Deribit dailies at 08:00 UTC
HAC_LAG = 1                 # 6h-of-30 overlap between adjacent expiries, zero beyond
DM_H = 2                    # n_eff = n/2, conservative
BLOCK = 2                   # moving-block bootstrap, absorbs lag 1
N_BOOT = 10000
ALPHA = 0.05
N_MIN_STAGE2 = 40
# stage 2 = expiries settling AFTER the pre-registration commit (de47191).
STAGE2_CUTOFF = pd.Timestamp("2026-08-01 00:00", tz="UTC")


def hourly_close() -> pd.Series:
    # hourly close series. Production candles are the primary source, BUT their
    ...
    m = _load(CANDLES_1M)
    # complete hours only (60 bars): a partial hour would give an early close.
...
def realized_rv(close: pd.Series, t0: pd.Timestamp, n_bars: int = H_BARS):
    # RV = sum of squared HOURLY log-returns over the n_bars hours after t0 — THE
    ...
    # ⚠ Pre-reg disambiguation, fixed BEFORE any result: the text says "over the
    # 30 hours from decision tick to expiry"; the operative horizon is
...
def build_panel(fc: pd.DataFrame, close: pd.Series) -> pd.DataFrame:
    # one observation per daily expiry. The decision tick is the LAST one with
    ...
    for d in days:
        ...
        if rv is None or rv <= 0:
            unobs["no_rv"] += 1
            continue
        # naive = trailing RV over the 30 hours BEFORE the tick (zero parameters).
...
def block_bootstrap_spearman(x: np.ndarray, y: np.ndarray, rng) -> tuple:
    # Spearman CI via moving-block bootstrap (block 2): adjacent expiries share
...
def hac_mean_test(z: np.ndarray, null: float) -> dict:
    # mean of z against `null` with HAC variance (Bartlett, lag 1) — the same
...
def main() -> int:
    ...
    # --count-only = SAFE sample monitoring, automatable. The n<40 -> NO_RUN guard
    ...
    # monitoring early exit. Building the panel IS the count — an expiry is
    ...
    # fail-fast anti-peeking guard: below n_min stage 2 computes NOTHING.
    ...
    # ② Spearman + block-bootstrap CI
    from scipy.stats import spearmanr
    rho = float(spearmanr(x, y).statistic)
    lo, hi = block_bootstrap_spearman(x, y, np.random.default_rng(12345))

    # ④ positive control: NN vs naive persistence in QLIKE
