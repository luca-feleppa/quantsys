# RETROSPECTIVE HEDGE DRY-RUN (B2/A1 pre-study, ROADMAP_VOL_BOOK) — offline,
...
# already-observed trades/positions:
#   1. per-interval option PnL: Δm_t = Δ(Σ leg mark) in BTC (side-adjusted);
#   2. per-interval inverse perp PnL with h_t BTC-equivalent position:
#      pnl = H_usd·(1/S_t − 1/S_{t+1}), H_usd = h_t·S_t (exact inverse formula);
#   3. TWO hedge-ratio conventions compared (B2 "delta convention" caveat):
#      δ_raw  = venue delta (greeks.delta, ∂V_usd/∂S)
#      δ_adj  = δ_raw − m_btc  (BTC terms: m=V/S → dm/dS=(Δ_usd−m)/S)
#   4. EMPIRICAL CHECK: regression of Δm on r=ΔS/S → slope = observed effective
#      delta, compared with mean(δ_raw) and mean(δ_adj);
#   5. metrics: hedged vs unhedged per-interval variance, estimated rebalancing
#      fees (perp taker, parametric), cumulative PnL.
# Does NOT touch 04b nor the pre-registered rule: it is the pre-study sizing
# the v2 hedged-vs-unhedged gate (no-trade band, expected drag).
import argparse
import json
import logging
import sys
...
# Deribit perp taker fee (fraction of traded notional) — parametric ASSUMPTION
...
def load_ticks(path: Path) -> list:
    # valid ticks = source 'position' (live position) with mark/underlying/delta
    ...
    for line in path.read_text(encoding="utf-8").strip().splitlines():
        ...
        if r.get("source") != "position":
            continue
        legs = r.get("legs", [])
        # the dry-run hedges the straddle: isolate the BODY (`body_idx`, default
        ...
        if len(legs) != 2 or any(
            ...
            continue
        # structure gamma (A12/ww): may be missing on illiquid strikes → None,
        ...
        ticks.append({
            ...
            "m": float(sum(l["mark"] for l in legs)),               # BTC per contract
            "delta_raw": float(sum(l["delta"] for l in legs)),      # venue convention
...
def perp_pnl_btc(h_btc: float, s0: float, s1: float) -> float:
    # inverse perp PnL in BTC: h BTC-equivalent position opened at s0, closed
    # at s1 → H_usd·(1/s0 − 1/s1), H_usd = h·s0. Exact, not linearized.
    return h_btc * s0 * (1.0 / s0 - 1.0 / s1)


def simulate(ticks: list, fee: float) -> dict:
    # group by structure (side/strike/expiry): rebalancing is intra-holding,
    # NEVER across two different positions.
    n = len(ticks)
    d_opt, d_hraw, d_hadj, rets, fees_raw, fees_adj = [], [], [], [], [], []
    # sided δ accumulated on the SAME sample as the regression (audit MINOR-3:
    ...
    def _close_hedge_fees():
        # hedge-leg CLOSING fee at structure end (audit MINOR-2: without it,
    ...
    for a, b in zip(ticks[:-1], ticks[1:]):
        if (a["side"], a["strike"], a["expiry_ms"]) != (b["side"], b["strike"], b["expiry_ms"]):
            ...
            continue
        side = a["side"]
        dm = side * (b["m"] - a["m"])                 # option PnL (BTC)
        # hedge = −net position delta, in both conventions.
        ...
        # fee on the REBALANCED notional (|Δh|; first tick pays the full opening).
    ...
    _close_hedge_fees()                               # end of series
    ...
    # empirical effective delta = OLS slope of Δm on r (Δm already side-adjusted):
    ...
    # slope SE (classic OLS) — with small n it decides whether the gap between
    ...
    out = {
        ...
        # total_net = hedged PnL net of fees (open+rebalances+close).
...
# ──────────────── no-trade band simulation (A4 POST_GATE_V1) ────────────────
def ww_band_offline(fee: float, S: float, gamma_struct: float, lam: float,
                    band_ref: float) -> float:
    # OFFLINE mirror of 04b's ww_band (A12, Whalley–Wilmott 1997): half-width
...
def simulate_band(ticks: list, fee: float, conv: str, band: float,
    ...
    # hedge-leg simulation WITH the no-trade-band hysteresis — same semantics
    ...
    h = None                       # current perp leg
    for a, b in zip(ticks[:-1], ticks[1:]):
        if (a["side"], a["strike"], a["expiry_ms"]) != (b["side"], b["strike"], b["expiry_ms"]):
            if h is not None:
                fees.append(fee * abs(h))          # structure close
            h = None
            continue
        side = a["side"]
        delta_conv = a["delta_raw"] if conv == "raw" else a["delta_raw"] - a["m"]
        if h is None:
            h = 0.0                                # new structure starts flat
        band_eff = band
    ...
    return {"conv": conv, "band": band, "band_mode": band_mode, "lam": lam,
            ...
            "var_reduction_vs_unhedged": None,     # filled by caller
            "fees_total": float(np.sum(fees)),
            "total_net": float(x.sum() - np.sum(fees))}


# A4 grids (POST_GATE_V1) — PRE-DECLARED before looking at the numbers:
...
def main():
    ...
    # A4 (POST_GATE_V1) — no-trade-band sweep with hysteresis + selection of
    ...
    # ww only under DOMINANCE (better net AND var↓): a net-only edge on the
