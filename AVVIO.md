...
🇮🇹 Pivot al timeframe **1h** (Strada 1 dopo il KILL del probe cross-sectional 2026-06-06, diagnosi "muro = magnitudine non segno"): a 1h il rapporto costo/σ per barra scende da ~1.9–3.3× a ~0.25–0.42× (il movimento di barra cresce ∝ √Δt, il costo roundtrip è fisso). Razionale econometrico in `TEORIA.md` §1, contratto interval in `CLAUDE.md` (regole scientifiche).

**EN** Pivot to the **1h** timeframe (Path 1 after the 2026-06-06 cross-sectional probe KILL, diagnosis "the wall is magnitude, not sign"): at 1h the per-bar cost/σ ratio drops from ~1.9–3.3× to ~0.25–0.42× (bar move grows ∝ √Δt, roundtrip cost is fixed). Econometric rationale in `TEORIA.md` §1, interval contract in `CLAUDE.md` (scientific rules).
...
| `scripts/01b_download_macro.py` | macro FRED/yFinance + walk-forward regime `RegimeMarkovBTC` (clock orario; su 7 anni ~3h); `--regime-only` rigenera SOLO `regime_probs.parquet`+`regime_hmm.pkl`+`regime_wf_checkpoint.pkl` (skip macro/normalizer/npz — il costo resta il walk-forward); `--regime-incremental` (B7) estende il parquet alle sole barre nuove dal checkpoint (0-1 fit MLE, minuti — `regime_hmm.pkl` NON aggiornato: solo il full rebuild rifà il fit finale full-sample); `--regime-bootstrap-checkpoint` ricostruisce una-tantum il checkpoint da pkl+parquet esistenti con golden test integrato (replay vs parquet, fail-fast); `--skip-regime` (2026-07-19) esegue la pipeline macro completa ma lascia il regime detector INTATTO (il full rebuild rimappa le etichette: da evitare a esperimenti aperti — le barre nuove si appendono con `--regime-incremental`) · FRED/yFinance macro + regime walk-forward (hourly clock; ~3h over 7 years); `--regime-only` regenerates ONLY `regime_probs.parquet`+`regime_hmm.pkl`+`regime_wf_checkpoint.pkl` (skips macro/normalizer/npz — the walk-forward remains the cost); `--regime-incremental` (B7) extends the parquet to new bars only from the checkpoint (0-1 MLE fits, minutes — `regime_hmm.pkl` NOT updated: only the full rebuild redoes the final full-sample fit); `--regime-bootstrap-checkpoint` one-off rebuilds the checkpoint from existing pkl+parquet with a built-in golden test (replay vs parquet, fail-fast); `--skip-regime` (2026-07-19) runs the full macro pipeline but leaves the regime detector UNTOUCHED (the full rebuild remaps labels: avoid while experiments are open — new bars are appended via `--regime-incremental`) |
...
🇮🇹 `python scripts/01c_iv_poller.py` (loop, default 10 min; `--minutes N` cadenza, `--once` smoke, `--backfill-dvol` storico DVOL orario 2021→oggi, `--greeks`: +3 chiamate/tick per i greeks del venue dello straddle ATM ~tenor-30h → `atm_greeks.parquet`, selezione identica a `pick_straddle` di `04b` — **ATTIVO sul VPS dal 2026-07-18 (C4), cadenza 5 min**; `atm_greeks` è nel pull/merge casa e nel health check) — 2 richieste pubbliche Deribit per tick, NESSUN account. Output append-only atomico in `data/iv/`: `chain/btc_options_YYYYMMDD.parquet` (snapshot raw, ~950 strumenti/tick), `atm_30h.parquet` (ATM IV delle 4 expiry vicine + IV interpolata in varianza totale a tenor costante 30h = forecast_horizon vol), `dvol.parquet` (controllo 30d). Scopo: storico IV short-tenor (non gratis altrove) per il gate **NN-RV vs IV implicita**.

**EN** `python scripts/01c_iv_poller.py` (loop, default 10 min; `--minutes N` cadence, `--once` smoke, `--backfill-dvol` hourly DVOL history 2021→today, `--greeks`: +3 calls/tick for the venue greeks of the ATM ~30h-tenor straddle → `atm_greeks.parquet`, selection identical to `04b`'s `pick_straddle` — **ACTIVE on the VPS since 2026-07-18 (C4), 5-min cadence**; `atm_greeks` is in the home pull/merge and the health check) — 2 public Deribit requests per tick, NO account. Atomic append-only output under `data/iv/`: `chain/btc_options_YYYYMMDD.parquet` (raw snapshot, ~950 instruments/tick), `atm_30h.parquet` (ATM IV of the 4 nearest expiries + total-variance-interpolated IV at constant 30h tenor = vol forecast_horizon), `dvol.parquet` (30d control). Purpose: short-tenor IV history (not free elsewhere) for the **NN-RV vs implied IV** gate.
