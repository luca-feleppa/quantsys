...
| ③ | **Monitoraggio linea vol** (CPU-only, **a scrittura zero**) | `scripts/vol/derive_mfiv.py` incrementale (**dopo** il merge per costruzione: legge la chain appena scaricata) + `scripts/vol/mfiv_comparator_judge.py --count-only` + contatori dei gate forward aperti (`executed` di `trades.jsonl` verso n≥30; **posizioni hedge-attive distinte** di `hedge_ledger.jsonl` verso n≥20 — `position_key` con ≥1 hedge eseguito, **NON** gli eventi di ledger: 1 posizione = open + N rebalance + flatten, e la 19JUL26 parzialmente hedgiata è esclusa come pre-dichiarato) + **copertura del file barre 1m** (vedi sotto) + **campione E1 stadio 2** (`edge_information_judge.py --stage 2 --count-only` verso n≥40, preceduto da fin dove arriva la serie dei close) · incremental MFIV derivation (after the merge by construction), `--count-only` expiry count, open forward-gate counters (option legs; **distinct hedge-active positions**, not ledger events), **E1 stage-2 sample** count preceded by the reach of the close series |

🇮🇹 ⚠ **Disciplina one-shot (pre-reg MFIV v2).** Il blocco ③ non esegue **mai** il giudice vero: `--count-only` calcola **solo timestamp** (nessun PnL, nessun edge, nessuna correlazione) e il giudice ha comunque il guard `n < 40 → NO_RUN` senza scrivere report → automatizzare il conteggio **non può produrre peeking**. Il **run one-shot dei giudici pre-registrati resta MANUALE**, alla prima sessione con n≥40. Fail-soft: ogni passo che fallisce logga un warning senza fermare la routine (`-SkipPull` salta ①, `-SkipMonitor` salta ③; `-Days N` = finestra del pull; `-RefreshCandles` **aggiunge** il passo ②bis, vedi sotto).
...
🇮🇹 ⛔ **`--hedge` è FALLITO — gate v2 chiuso il 2026-08-11 (FAIL 2/3): l'invocazione di produzione torna a `--execute` nudo.** La varianza per-trade scendeva del 55.3% (① superata con margine) ma il drag valeva −0.647·SE contro un budget di −0.25·SE, per **76% fee perp e 0.9% funding**. Dettaglio e decomposizione: `TEORIA.md` §12.2. ⚠ **Vincolo di ORDINE nel disattivarlo, non cosmetico:** `maybe_hedge` e `reconcile_hedge_state` girano **solo** dentro il ramo `--hedge`, quindi riavviare il servizio senza il flag mentre `results/vol_paper/hedge_state.json` esiste lascia la leg perp **nuda, non gestita e mai flattenata** sul testnet. Si disattiva **dopo un settlement**: verificare nel `hedge_ledger.jsonl` l'evento di flatten `reason="settled"` e la sparizione di `hedge_state.json`, poi riavviare. La leg opzioni è identica con o senza hedge (path v1 bit-identico), quindi la disattivazione non perturba i campioni forward aperti.

**EN** ⛔ **`--hedge` has FAILED — v2 gate closed on 2026-08-11 (FAIL 2/3): the production invocation goes back to a bare `--execute`.** Per-trade variance fell 55.3% (① passed with margin) but the drag was −0.647·SE against a −0.25·SE budget, made of **76% perp fees and 0.9% funding**. Detail and decomposition: `TEORIA.md` §12.2. ⚠ **An ORDERING constraint when disabling it, not a cosmetic one:** `maybe_hedge` and `reconcile_hedge_state` run **only** inside the `--hedge` branch, so restarting the service without the flag while `results/vol_paper/hedge_state.json` exists leaves the perp leg **naked, unmanaged and never flattened** on the testnet. Disable it **after a settlement**: verify the `reason="settled"` flatten event in `hedge_ledger.jsonl` and that `hedge_state.json` is gone, then restart. The options leg is identical with or without the hedge (bit-identical v1 path), so disabling does not perturb the open forward samples.
