...
- **B7 — walk-forward regime INCREMENTALE — ✅ IMPLEMENTATO 2026-07-15 sera** (`macro/regime.py` + `01b`): checkpoint di catena `data/regime_wf_checkpoint.pkl` (parametri ultimo retrain + PCA sign-aligned + posteriore filtrato + cadenza + scaler congelato) salvato a fine full rebuild; `01b --regime-incremental` = append delle sole barre nuove (0-1 fit MLE → minuti vs ~3h), fail-fast su ogni incoerenza checkpoint↔parquet↔config, ordine di scrittura parquet→checkpoint (crash-safe); `--regime-bootstrap-checkpoint` = ricostruzione one-off da pkl+parquet con golden replay-validation (nessun artefatto scritto su divergenza). Vincoli rispettati: loop Hamilton NON toccato (stesso metodo, zero copie); `continue_walkforward` è un MIRROR dichiarato del blocco retrain (divergenza intercettata dal golden test); refit paralleli/GPU restano RESPINTI. Golden test sintetico `tests/test_regime_incremental.py` 6/6 (bit-parity full vs troncato+continuazione via `_stop_at`, guard fail-fast, roundtrip checkpoint). Deviazione accettata e documentata (TEORIA.md): scaler congelato al full rebuild → re-ancoraggio con full rebuild periodico. Nota VPS invariata: decisione accodata alla migrazione 04b.
...
**FATTIBILITÀ HARDWARE — PROBE VRAM ESEGUITO 2026-07-14 (RTX 2070S 8GB): il train congiunto ENTRA COMODAMENTE.** Probe con costruzione identica a `main()` (CAFN default d_model=64/2 layer + 3 downstream `cafn_downstream_d_model=64`, T=120 F=104 d_latent=16, 2.78M parametri totali, fp32, backward congiunta, AdamW): **batch 64 → picco 1.537 MB** (0.16 s/step, epoca 51.130 finestre ≈ 2 min), batch 32 → 787 MB, batch 16 → 429 MB. Il timore OOM (punto 3 sopra) era calibrato sui modelli production full-size: i downstream di 02d sono deliberatamente ridotti → **nessuna mitigazione necessaria, batch 64 di default va bene**. **DECISIONE PRE-RUN (2026-07-14, zero numeri visti): `cafn_downstream_d_model` 64→128 in `config/cafn.yaml`** — larghezza production, riduce il gap di transfer del gate (interpretazione asimmetrica dichiarata: FAIL a scala ridotta = kill forte; PASS ≠ promozione, serve conferma a scala/loss production). Restano dichiarate le semplificazioni: n_macro=0, giudice MSE-μ, 2 layer. Il gate pre-registrato (metrica ≥3% MSE-μ su ≥2/3 archi) è invariato. Leve di riserva se la config downstream crescesse (in ordine):
1. **`--batch` ridotto 64→32→16** (leva sanzionata dallo script; memoria ~lineare nel batch; il gate resta valido perché CAFN e baseline NO-CAFN condividono i settaggi — scelti PRIMA per ragioni di memoria, mai ritoccati a risultati visti);
2. **probe `--max-steps 10` + `torch.cuda.max_memory_allocated()`** prima di ogni run vero (costa 2 min, evita OOM a metà training);
3. **gradient accumulation o backward per-arch** (la loss è Σ per arch → backward separata per modello libera il grafo di ciascun arch: picco = CAFN + 1 modello invece di 3; patch da applicare IDENTICA a entrambi i bracci del gate);
4. **AMP: NO** — su TCN-Mamba il training AMP è stato deliberatamente disattivato (fix 2026-05-15), non reintrodurla in un run congiunto che include Mamba;
5. **ultima ratio `--device cpu`** (zero vincolo VRAM, 8 thread, ~10-30× più lento → run notturno).
