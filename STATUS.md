...
> Prima di iniziare qualsiasi task: **leggi questo file**. Protocollo sperimentale e corpus dei risultati negativi: `THEORY.md` §12.
...
canonico e il vintage datato qui; (3) contatore E1 **ritirato il 17/09** da `avvio_sessione.ps1`
(blocco ③ e check di freschezza della serie close, parse PS pulito), `START` §5.3, `scripts/README` e
`docs/ROADMAP_VOL_BOOK`, EN+IT; resta — solo al go-live — l'aggiunta di `ft1_execution_judge.py --count-only` alla routine;
...
`n = 83`; (c) producing a vintage with
...
dated here; (3) E1 counter **retired on 09-17** from `avvio_sessione.ps1` (block ③
and the close-series freshness check, clean PS parse), `START` §5.3, `scripts/README` and
...
3. **Ritiro del contatore E1 dalla routine** (`avvio_sessione.ps1` blocco ③, `START.md` §5.3): gate
...
4. **Deploy del pacchetto `04b` sul VPS senza `--adaptive`** (inerte, path v1 bit-identico): solo su
   istruzione esplicita, procedura di deploy in `START.md` §5.3bis.
...
3. **Retire the E1 counter from the routine** (`avvio_sessione.ps1` block ③, `START.md` §5.3): gate
...
4. **Deploy the `04b` package to the VPS without `--adaptive`** (inert, bit-identical v1 path): only
   on explicit instruction, deployment procedure in `START.md` §5.3bis.
...
regola scritta in `START.md` §5.3. (b) `timeout_after_<ultima gamba>` smonta una struttura completa pagando lo spread
...
directory) make possible: rule written in `START.md` §5.3. (b) `timeout_after_<last leg>` unwinds a complete structure paying the spread twice: it is
...
> stadio 2 one-shot eseguito a `n = 40`, esito scritto in `STATUS.md` + `THEORY.md` §12.3-bis,
...
> at `n = 40`, outcome written to `STATUS.md` + `THEORY.md` §12.3-bis, `edge_information_stage2.json`
...
2. **Ordine ≥ 2 antisimmetrico e ordine 3 — nullo per la DICOTOMIA PARI/DISPARI.** L'area di
   ...
   informazione **dispari**. La sintesi trasversale di §12 (`THEORY.md`) dà i momenti dispari
...
numero come **decisione**, e solo dopo si aggiornano `THEORY.md` §12.2 **e** il § STATO NOTO
...
update `THEORY.md` §12.2 **and** the auto-loaded copy's KNOWN STATE section (the two copies of
...
(`THEORY.md` §12 e la copia auto-caricata) e il filone si chiude **a questo scale**; non si
...
🇮🇹 **`THEORY.md` §12.2 + `README.md`** (commit `736f29b`) — la sezione nomina **quattro**
...
**EN** **`THEORY.md` §12.2 + `README.md`** (commit `736f29b`) — the section names **four**
...
- **Cambiato:** `THEORY.md` §12.2 e il corpus KILL della copia auto-caricata hanno una voce
...
**EN** **Changed:** a new KILL entry in `THEORY.md` §12.2 and in the auto-loaded copy;
...
| Contatore | 24/08 | **25/08** | Soglia | Nota |
|---|---|---|---|---|
| E1 stadio 2 | 20 | **20** | 40 | invariato, ed è la conseguenza attesa (①) |
| L2 h=30 (`n_eff`) | 27.5 | **28.2** | (216) | run contiguo **996h**, nessun buco in 7g |
| leg opzioni | 53 | **54** eseguite (55 righe) | (30) | gate chiuso il 30/07, FAIL 0/3 |
| barre 1m (lag) | 23.5g | 24.4g | — | non è un debito (decisione 10/08) |

Derivazione MFIV incrementale: +251 righe → **12.313**. Wedge MFIV−ATM **mediana +3.11** vol pt (p10 +2.13, p90 +4.94): la mediana resta stabile al centesimo, la coda alta rientra (p90 +4.97 → +4.94) dopo l'allargamento letto ieri. Colonna diagnostica, contatore ritirato dal 18/08.

### ① Il conto E1 è fermo su TUTTE le voci, non solo sul totale — e questo chiude la lettura di ieri

Il blocco ③ ha ristampato «serie close STALE → conteggio SOTTOSTIMATO» (lag **104h**, era 83h). Nessuna azione, per la stessa aritmetica di ieri: mancano 20 osservazioni, il tetto odierno ne vale al più 5.

La novità utile è che **le voci non osservabili non si sono mosse affatto**: `no_tick: 35` e `no_rv: 5`, identiche a ieri, su una finestra identica (`2026-08-01 → 2026-08-20`). Ieri `no_rv` era passata da 2 a 5 in tre giorni e la voce ① l'aveva letta come «cadenza ≈ 1/giorno»: **non lo è**. Con la serie close ferma la finestra di analisi non avanza, quindi non entrano expiry nuove e nessuna voce può cambiare — il +3 di ieri veniva dal merge del chain che riempiva tick *dentro* la finestra già esistente, non dal suo avanzamento. La cadenza apparente era un artefatto di aggregazione su tre giorni non aperti.

Conseguenza pratica: **l'intera contabilità E1 è funzione di `raw_candles.parquet`**, non solo il totale. Una proiezione sul calendario resta sbagliata per costruzione, e il tetto «al più 25/40» va ri-misurato al refresh, non estrapolato.

**Stima della data invariata:** ~1 osservazione per giorno di *serie close* ⇒ close a **~9-10/09**, che è anche la data del refresh obbligatorio (prerequisito ⑤ della pre-reg E1).

### ② Regime B7 fresco — 0 barre nuove, come previsto ieri

`regime_probs.parquet` e `raw_candles.parquet` restano entrambi al **20/08 15:00**: 0 barre nuove < 168, nessun refresh incrementale. È la conseguenza attesa registrata ieri — il refresh B7 consuma candele orarie e senza refresh candele non ce ne sono. Il prossimo non può presentarsi prima del refresh candele di ~9-10/09.

### ③ Pagina delle architetture — decisione ancora aperta, terza sessione

Il file esiste ancora nella cartella temporanea della sessione del 21/08. La decisione (portarla nel repo come documentazione bilingue IT/EN paragrafo per paragrafo, oppure lasciarla fuori dai tracciati) non è scaduta ma resta materiale effimero: la cartella non è garantita. Se la si vuole, va spostata sotto il repo prima che la cartella venga ripulita.
...
**Sessione di routine.** Nessun gate in soglia, nessun one-shot, **nessun refresh candele**, zero GPU. Scritture: solo le derivazioni incrementali della routine (`mfiv_30h.parquet`). Npz, scaler, `PipelineState`, `raw_candles.parquet`, `regime_probs.parquet`, vintage macro `20260730`: **invariati**.

⚠ **Le sessioni del 22/08 e 23/08 non sono state aperte:** questa voce copre **tre** giorni. Come il 19/08, ciò che ne risente è la sola lettura dei delta — nessun contatore è perso, perché i contatori si ri-misurano e non si accumulano a mano.

**Routine 24/08 — exit 0.** 4 collector freschi (IV 0.0h · L2 0.0h · trades 0.0h · `04b` 1.0h). Macro: vintage `20260730` invariato sui due lati, **nessuna promozione**. Merge: chain +169.256 (21/08, file già presente) e tre file nuovi 22/08 +301.610, 23/08 +290.180, 24/08 +12.974; L2 +9.540/+17.279/+17.280/+739; trades +11.772/+6.377/+6.296/+215; `forecasts` 1019→1082, `trades.jsonl` 52→54 (+2), `exec_diag` 935→1001 (+66), `hedge_ledger.jsonl` **+0** (atteso, leg spenta dal 13/08); `hedge_state.json` assente sul VPS.

| Contatore | 21/08 | **24/08** | Soglia | Nota |
|---|---|---|---|---|
| E1 stadio 2 | 20 | **20** | 40 | invariato: nessun refresh candele in 3 giorni (①) |
| L2 h=30 (`n_eff`) | 25.4 | **27.5** | (216) | run contiguo **975h**, nessun buco in 7g |
| leg opzioni | 51 | **53** | (30) | gate chiuso il 30/07, FAIL 0/3 |
| barre 1m (lag) | 20.9g | 23.5g | — | non è un debito (decisione 10/08) |

Derivazione MFIV incrementale: +746 righe → **12.062**. Wedge MFIV−ATM **mediana +3.09** vol pt (p10 +2.12, p90 +4.97): la mediana è stabile al centesimo da sei sessioni, la coda alta si allarga (p90 +4.54 → +4.97). Colonna diagnostica, contatore ritirato dal 18/08.

### ① Warning E1 ristampato, nessuna azione — e stavolta il limite superiore è quantificato e basta a decidere

Il blocco ③ ha ristampato «serie close STALE → conteggio SOTTOSTIMATO» (lag **83h** contro soglia fissa 6h). Trattato come sempre da inferenza da proxy: la serie close ferma al **20/08 14:00** è vera, la conseguenza va derivata.

Le non osservabili sono `no_tick: 35` — **nessun refresh le tocca**, non esiste un tick qualificante nella finestra — e `no_rv: 5` (era 2 il 21/08: +3 in tre giorni, cadenza ≈ 1/giorno coerente con lo storico). Quindi **5 è il limite superiore** di ciò che un refresh potrebbe sbloccare oggi: il contatore arriverebbe al più a **25/40**, non a 40.

Non lanciato, e la ragione è aritmetica prima che disciplinare: mancano 20 osservazioni e il tetto odierno ne vale 5. Il refresh **obbligatorio** resta quello immediatamente precedente al one-shot (prerequisito ⑤ della pre-reg E1), dove serviranno le expiry di allora. Ogni scrittura su `raw_candles.parquet` è un atto, non un automatismo.

**Stima della data invariata, e ora ricalibrata sul tasso misurato.** Il contatore è funzione dell'avanzamento della serie close, non del calendario: 12 (close 12/08) → 17 (close 18/08) → 20 (close 20/08 15:00) ⇒ ≈ **1 osservazione per giorno di serie close**. Per 20→40 servono ~20 giorni di avanzamento ⇒ close a **~9-10/09**, che è anche la data del refresh obbligatorio.

### ② Regime B7 fresco — 0 barre nuove, ed è la conseguenza attesa

`regime_probs.parquet` è al **20/08 15:00** (`n_bars` 66.928) e `raw_candles.parquet` pure (66.868 barre, ultima `open_time` 20/08 15:00): **0 barre nuove < 168**, nessun refresh incrementale. È corretto per costruzione — il refresh B7 consuma candele orarie, e senza refresh candele non ce ne sono di nuove. Il prossimo refresh B7 non può quindi presentarsi prima del refresh candele di ~9-10/09.

### ③ Pagina delle architetture — ancora su disco, decisione ancora aperta

La voce ⑤ del 21/08 la dava per persa a fine sessione. **Verificato: il file esiste ancora** nella cartella temporanea di quella sessione. La decisione è quindi ancora esercitabile e non è scaduta: portarla nel repo come documentazione **bilingue IT/EN paragrafo per paragrafo** (lavoro reale, e pagina pubblica su GitHub) oppure lasciarla fuori dai tracciati. Finché non è presa, resta materiale effimero — la cartella non è garantita.
...
**Fermi e non sbloccati:** E1 stadio 2 **20/40** (~9-10/09), L2 `n_eff` 27.5/216 (~fine novembre), refresh macro bloccato fino alla chiusura di E1, direzionale e lever di training vol classi chiuse, `--hedge` FALLITO (11/08), comparatore MFIV FALLITO (18/08), filone strip CHIUSO ex-ante (21/08).
...
- **Propagato** in `THEORY.md` §12.2 (IT+EN, copia pubblica), corpus KILL, `CHANGELOG.md`, `RIPRESA.md`, `POST_GATE_V1.md`, `START.md`.
...
- **FAIL** → il comparatore live resta **ATM IV**, MFIV resta colonna diagnostica permanente, **item chiuso**. Va scritto comunque, e va scritto in **entrambe** le copie del corpus KILL (§ STATO NOTO del manifesto operativo **e** `THEORY.md` §12 — la copia pubblica; se divergono ha ragione `THEORY.md`).
...
`THEORY.md` §12.2 (paragrafo dedicato IT+EN, con la decomposizione, il contro-fattuale a fee nulle e la qualificazione sul t appaiato), `CHANGELOG.md`, indice KILL del contesto di sessione. Il report macchina è `results/vols/hedged_vs_unhedged.json` — **quello** è il record, non questa prosa.
...
Suite **491 passed / 1 skipped** (era 490/1). Doc allineata: `START.md` §5.3 (riga ③ + nuovo paragrafo IT+EN sul guard che protegge solo sotto soglia), `scripts/README.md`.

### 🚩 `-RefreshCandles` — l'opzione (b) esiste, ma come atto e non come default

Audit letto nel codice, non nella doc che lo descrive. **Meccanicamente l'automazione sarebbe sicura**, più di quanto avessi detto proponendo la scelta: (i) **no-op** se non c'è nulla di nuovo — `next_ts >= now_floored → return`, poi `n_new == 0 → sys.exit(0)` **prima** della scrittura, mtime invariato (esercitato: il run di verifica ha stampato `66.530 → 66.530, +0 nuove`); (ii) `drop_duplicates(subset="open_time")` tiene la **prima** occorrenza, quindi le righe esistenti vincono e **la storia non si riscrive mai**; (iii) `now_floored = floor(utcnow) − 1 barra` e filtro del delta → mai la barra in formazione; (iv) scrittura atomica; (v) **il file non arriva mai al VPS** — l'unico trasferimento casa→VPS in `pull_vps_data.ps1` è la macro, tutto il resto è `scp` in direzione opposta.

**Tre ragioni per cui resta comunque un flag e non un default**, tutte trovate scritte nel repo e non dedotte:

| | ragione |
|---|---|
| ① | il blocco ③ è descritto come *«CPU-only e OFF-PATH: nessun file di produzione toccato»*. È una **categorica**, ed è ciò che permette a un lettore futuro di fidarsi che aprire una sessione non perturbi un esperimento. Sostituirla con «scrive, ma innocuo» obbliga a rivalutare il danno ogni volta |
| ② | **composizione di automatismi:** estendere le barre fa avanzare lo staleness B7, e a ≥168 barre nuove la routine lancia da sola `01b --regime-incremental`, che scrive `regime_probs.parquet` e il checkpoint. Oggi siamo a **95/168**, cioè ~3 giorni dall'innesco |
| ③ | **decisivo:** con l'estensione automatica **congelare i dati diventa impossibile**. Il blocco ② è progettato sul caso (*«con candele congelate il check stampa fresco ed è un no-op»*) e la pre-reg A8-bis dichiara l'invariante alla lettera — *«Candele/npz/regime_probs NON si toccano fino a chiusura gate»*. Congelare passerebbe da **non fare nulla** a **ricordarsi `-SkipMonitor`**, che è la forma esatta della promozione macro avvenuta *de facto per automazione* il 31/07 |

**④ Corollario di provenienza, la versione sottile del ③.** `train_end = int(n_total · (1 − val_frac − test_frac))`: lo split è una **frazione del conteggio righe**, quindi ogni barra appesa sposta i confini train/val/test al prossimo rebuild dell'npz. Automatizzando, il vintage del dataset diventerebbe funzione di **quante sessioni sono state aperte** invece che di una data dichiarata — cioè smetterebbe di essere dichiarabile, che è la proprietà per cui esiste M1 sull'asse macro. È la stessa nota di R1 (*«irrilevante finché l'npz non viene rigenerato, decisivo se lo si rigenera»*) letta in avanti.

⚠ **Qualificazione onesta, in direzione opposta:** oggi **nessuna pre-registrazione aperta congela le candele** — E1 ⑤ ne *impone* il refresh, MFIV v2 è model-independent, il congelamento della v2 hedged riguarda i parametri di hedge. Installare (b) non avrebbe violato nessun gate **aperto**: l'obiezione riguarda il prossimo esperimento a dati congelati e la provenienza, non lo stato corrente.

**Forma adottata:** `-RefreshCandles`, spento di default, modellato su `-PromoteMacro`. Passo **②bis**, collocato **dopo** il check B7 di proposito: così l'eventuale refresh incrementale del regime parte al **prossimo** avvio e le due scritture restano separate e ognuna visibile, invece di concatenarsi in una sola sessione. **Quando serve:** prima di annotare il contatore E1 se il blocco ③ stampa il warning di ritardo, e prima del run one-shot di E1 stadio 2. ⚠ Verificato leggendo i giudici: **fra i gate aperti solo E1 legge le barre** — `hedged_vs_unhedged_judge` legge `trades.jsonl`, `hedge_ledger.jsonl` e il funding perp; `mfiv_comparator_judge` legge il chain e `forecasts.parquet`. Quindi il flag ha un solo consumatore identificato, non un perimetro vago.
...
Propagato in: `THEORY.md` §12.2 (titolo IT+EN, paragrafo DM, paragrafo di provenienza, **nuovo paragrafo «Precisione della banda» IT+EN**), `README.md` §5.1 (IT+EN), commento di intestazione di `tests/test_har_c_baseline.py`, § STATO NOTO del manifesto. **NON toccati i record datati** — le voci C2 (30/07) e C3 (31/07) di `THEORY.md`, `CHANGELOG.md` e `RIPRESA.md` dicono quale banda fu decisa *allora* ed erano vere quando furono scritte: un diario non si riscrive. Il titolo di §12.2 dichiara la ri-espressione e la data, ed è la prima riga che si legge.
...
| comandi di setup + flag di `run_all.py` | `README.md`, `START.md`, `run_all.py --help` |
...
2. `THEORY.md` §12.2 perde la frase «e oggi nulla lo segnala»; il buco passa da **dichiarato** a **chiuso**, con il limite del controllo positivo sintetico scritto accanto.
...
- In ogni caso l'esito si scrive: qui, in `THEORY.md` §12 e in `CHANGELOG.md`.
...
**La banda pubblicata non cambia di una cifra:** il numeratore della coppia canonica coincide **esattamente** con quello già in `THEORY.md` §12.2 (val 0.26143, test 0.23637). Cambia lo **statuto** del claim, non il valore.
...
**▶️ AZIONE ESATTA DA CUI RIPARTIRE.** Nessun gate aperto in scadenza. **Passo 5 del protocollo completato: entrambe le sandbox eliminate** (`models_r1b_sandbox/` e `models_r1_sandbox/`, ~40 MB). Prima di cancellare la seconda sono stati verificati byte-identici checkpoint e `pipeline_state` contro l'artefatto (md5), e sono stati **recuperati nell'artefatto** i tre file che vivevano solo lì: `reliability_diagram.png` (ECE 0.076), `test_predictions.npz` e `experiment/{config.yaml,meta.json,results.json}` — da cui il **commit di addestramento `6a8934e`**, cioè lo stato *precedente* ai fix di oggi, che sono arrivati dopo il training e non toccano i pesi. `PROVENANCE.md` ora dichiara commit, config di training e **vintage macro V2 (30/07 20:55:02)**, quest'ultimo perché il guard non lo copre. Prossimo evento reale invariato: giudice `hedged_vs_unhedged_judge.py` a n≥20 (oggi 15, +1/giorno), **~08-09/08, run MANUALE**. Comparatore MFIV v2 a 28/40, ~2 settimane.

**Domani (2026-08-05): giornata senza gate.** L'ordine è (1) `.\avvio_sessione.ps1` con redirezione a livello di OS, (2) annotare i quattro contatori — attesi hedged ~16/20 e MFIV ~29/40 — e nient'altro di dovuto. **L'unico lavoro con un oggetto preciso** è la pre-registrazione del gate che estende l'impronta di identità al **vintage macro**: oggi il guard lascia passare `matches: true` su modelli addestrati con macro diverse, e lo scarto misurato è 0.0019 sul rapporto pubblicato. ⚠ Va **pre-registrata e non eseguita di slancio**: tocca l'impronta che decide se un numeratore è confrontabile, quindi tocca indirettamente il numeratore pubblicato. Scaffold: skill `/preregister`. Se invece si preferisce non aprirlo, la giornata è di sola routine — e va bene così, perché nessun contatore è in soglia.
...
2. La banda pubblicata è **riscritta con i numeri della coppia canonica, quali che siano**, arrotondata al punto percentuale come oggi; `THEORY.md` §12.2 perde la qualificazione "non riproducibile alla cifra" e guadagna il puntatore all'artefatto. Il §12.2 va aggiornato **anche se i numeri coincidono**: cambia lo statuto del claim, non solo il valore.
...
⚠ **Perché contano più di qualunque ottimizzazione:** nessuno dei due costava un secondo. Entrambi trasformano un fallimento muto in uno esplicito, che è la stessa logica dei guard già in `THEORY.md` §12.5 (σ, `forecast_horizon`, `interval`, scaler) — ora estesa ai due punti che ne erano scoperti.
...
3. **Niente fonte parallela** — ogni scheda punta a una sezione **esistente** di `THEORY.md`, verificata a ogni build. Su divergenza registro↔TEORIA, **TEORIA ha ragione**.
...
4. **`THEORY.md` §12.2**: paragrafo bilingue sulla provenienza del numeratore e sul limite di riproducibilità. **Nessun numero della banda è stato riscritto.**
...
**Test:** `tests/test_scaler_identity_guard.py` **6/6**, di cui uno **sui file veri**: verifica che il guard veda il disallineamento storico. Se un giorno il modello venisse riaddestrato sull'npz corrente quel test cade — di proposito, per obbligare ad aggiornare anche la qualificazione in `THEORY.md`.
...
- **In ogni caso scritto**, con i numeri, in `STATUS.md` e in `THEORY.md` §12.
...
**Controllo:** run senza alcun flag → **103 chiavi identiche** al report C3 a flag accesi, 0 differenze, unica aggiunta `har_cj.har_c.ratio_nn_over_har_c` (rapporto del claim, prima ricavabile solo a mano); `metrics` e `gate` invariati, verdetto PASS, ratio NN/HAR-RV 0.7695. Le quattro condizioni di C2 restano calcolate come **monitoraggio continuo** (② segnala a ogni run se il claim regge ancora contro la baseline forte), non come condizioni vive. Suite **364 passed / 1 skipped**. Doc allineate: `THEORY.md` §12.2 (IT+EN), `scripts/README.md`, tabella env del manifesto.
...
- **In ogni caso l'esito è scritto**, in `STATUS.md` **e** in `THEORY.md` §12 (doppia copia da tenere in sincrono, direttiva #2).
...
4. **La qualificazione ⑥ di C2 passa da "interpretazione non dimostrata" a "interpretazione FALSIFICATA"**, con i numeri. Il **verdetto** di C2 resta valido: HAR-CJ *è* più forte di HAR-RV fuori campione, e il claim vi sopravvive — semplicemente non per la ragione che le era stata attribuita. `THEORY.md` §12.2 e `README.md` aggiornati nominando la baseline corretta.
...
**Vincolo anti-goalpost (pre-dichiarato).** Qualunque sia il segno, il numero viene riportato. Se ② **fallisce** — cioè il NN non batte HAR-CJ del 5% — allora **il claim pubblicato era in misura sostanziale un artefatto di baseline debole**, e va riscritto di conseguenza in `README.md` e `THEORY.md` §12.2, nominando esplicitamente la baseline. Non è una ritrattazione del PASS 2026-06-10, che resta valido *rispetto alla baseline con cui fu registrato*: è una **ri-specificazione** del confronto, e va presentata come tale. Nessuna soglia viene rinegoziata a risultati visti.
...
- **Vincolo anti-goalpost onorato:** il numero è riportato con il suo segno, qui e in `THEORY.md` §12.2. Non diventa la banda pubblicata perché la ① — condizione di adozione scritta **prima** di vedere i numeri — non è soddisfatta, non perché sfavorevole al NN.
...
**⑥ Igiene.** Suite piena: **341 passed, 1 skipped** (era 320 totali; +22 test nuovi). Invariante di attribuzione verificato vuoto. `results/vols/qlike_report_1h_val.json` è stato rigenerato sul vintage corrente (il file su disco era del 07-19, **antecedente di 12 minuti** all'npz attuale: era stale, ora è il run flag-off riproducibile). Doc sincronizzate: `THEORY.md` §12.2 (C1 implementato-inerte) e §12.4 (A10 come ultimo candidato aperto), `START.md` §4.3, `scripts/README.md`, `README.md` (conteggio test 320 → 342).
...
**④ Residuo NOTO, non toccato (fuori dal perimetro dell'intervento).** `THEORY.md` §7 deriva la **t-Student NLL** ma **non** la pinball/quantile loss, che è il default di produzione; la nota di notazione in testa definisce μ/σ/ν come parametri della densità t-Student senza menzionare il path quantile. Asimmetria documentale reale: la loss effettivamente in uso è l'unica senza derivazione. Candidato a un §7.x, non urgente, nessun impatto sul codice.
...
**⑥ §7.0 DI `THEORY.md` — la loss di produzione ora è documentata, e nel documentarla sono emerse due cose non note.** L'intervento nasce dal residuo ④ (pinball non derivata) ma non si è fermato lì.
...
- Doc-sync: README (tree `model/`+`data/`), AVVIO (greeks ATTIVO), THEORY.md (pattern architetturali).
...
- **Doc sync:** header `04b` (lista output), `START.md` (tree results/), `ROADMAP_VOL_BOOK.md` (A6 marcato ✅ IMPLEMENTATO). README/TEORIA non impattati (nessuna architettura cambiata).
