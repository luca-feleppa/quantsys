...
- **B7 — walk-forward regime INCREMENTALE** (`macro/regime.py` + `01b --regime-only`, aggiunto 2026-07-15): oggi ogni refresh di `regime_probs.parquet` è un full-rebuild O(n²) (~3h su 7 anni: ~30 refit MLE expanding di costo O(t) — misurato 2026-07-15). Design: persistere i parametri per-refit (θ_k a ogni t_k) accanto a `regime_hmm.pkl`; al refresh caricare l'ultimo θ + le probs esistenti, stimare SOLO i refit-point nuovi oltre lo span coperto e filtrare in avanti le barre nuove (append) → estendere "da 06-22 a oggi" costerebbe **minuti, non ore**. **Bit-parity by construction**: l'append preserva la catena sequenziale dei warm-start (stesso ordine di refit del full rebuild) — golden test obbligatorio: incrementale == full rebuild sullo stesso span, bit-per-bit su righe nuove E righe preesistenti. ⚠ Vincoli: il loop Hamilton per-barra è zona DA-NON-TOCCARE (la ricorsione resta identica, si tocca solo l'orchestrazione dei refit); l'alternativa "refit paralleli su 8 thread" è RESPINTA (romperebbe la catena warm-start → risultati non bit-identici). GPU valutata e respinta 2026-07-15: ricorsione sequenziale K=3, kernel-launch overhead dominante, statsmodels CPU-only. **Nota VPS:** spostare il refresh sul VPS ha senso SOLO se/quando `04b` migra sul VPS post-gate E il regime diventa consumer live (es. gating v2): oggi il consumer è episodico e locale (esperimenti A3), il VPS ha 2 vCore condivisi già occupati dai collector, e con l'incrementale il refresh locale costa minuti → accodare la decisione alla migrazione 04b, non prima. Effort S-M.
