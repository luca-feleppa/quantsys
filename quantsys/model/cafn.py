...
Coordination layer upstream of the project's three forecasting models
...
⚠ SCIENTIFIC HONESTY (THEORY.md §12): the "causal penalty" is a **regularizer**
...
# ─── Causal flow attention block ──────────────────────────────────────────────
class CausalFlowAttention(nn.Module):
    # multi-head self-attention with a strictly causal (lower-tri) mask.
    ...
    def forward(self, h: torch.Tensor):
        # h: (B, T, d_model)
        T = h.shape[1]
        # bool mask (True = disallowed) — t may only attend to s ≤ t.
    ...
    def _causal_flow_penalty(A: torch.Tensor) -> torch.Tensor:
        # A=(B,T,T), row t is a distribution over s≤t (sums to 1 over unmasked).
        B, T, _ = A.shape
        idx = torch.arange(T, device=A.device, dtype=A.dtype)
        # normalized lag (t-s)/(T-1) ≥ 0 on the causal triangle.
        lag = (idx[:, None] - idx[None, :]).clamp(min=0.0) / max(T - 1, 1)
        proximity = (A * lag).sum(dim=-1).mean()                 # penalize far past
        if T > 1:
            stability = (A[:, 1:, :] - A[:, :-1, :]).abs().mean()  # penalize jumps
        else:
...
class CausalAttentionFlowNetwork(nn.Module):
    # Filter → projection → stack of causal blocks → per-timestep latent.
    ...
    def __init__(self, n_features: int, d_model: int = 64, n_heads: int = 4,
        ...
        # denoising FILTER — LayerNorm + sigmoid per-feature gate: scales raw
        ...
        # learnable positional embedding (causal → temporal order matters).
    ...
    def forward(self, x: torch.Tensor, extra: torch.Tensor = None):
        # x: (B, T, F)  ·  extra: (B, T, F_extra) optional
        ...
        # filter → per-feature gate (sigmoid) → projection + positional.
    ...
    def encode(self, x: torch.Tensor, extra: torch.Tensor = None) -> torch.Tensor:
        # latent only (eval), handy for inference/diagnostics.
        self.eval()
