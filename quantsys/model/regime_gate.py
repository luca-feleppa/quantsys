"""A3 — Construction of the causal regime gate for the regime-MoE head.

Aligns the RegimeMarkovBTC filtered probabilities (`data/regime_probs.parquet`,
...
`[t, t+1h)` (log_ret/log_rv label-left resample) → it becomes AVAILABLE only at
`t+1h`. The index is therefore SHIFTED to availability time (+1 hourly bar — the
regime detector clock is hourly by design) BEFORE the BACKWARD `merge_asof`: last
...
# gate columns in the parquet (order = regime index R0/R1/R2).
GATE_COLS = ["regime_prob_0", "regime_prob_1", "regime_prob_2"]

# canonical path of the regime parquet (produced by 01b_download_macro).
DEFAULT_REGIME_PARQUET = "data/regime_probs.parquet"


# availability lag of a regime row (hourly detector clock by design: row `t`
# = bar [t, t+1h) → available at t+1h). BLOCKER-1 audit.
AVAILABILITY_LAG = "1h"

# max age of the last available regime before the uniform fallback (bounded
# staleness, MAJOR-1 audit); beyond 20% stale samples → fail-fast.
DEFAULT_MAX_AGE = "168h"
STALE_FRAC_FAIL = 0.20


# Builds the (N,3) float32 gate array aligned to the sample timestamps.
def build_regime_gate(timestamps,
                      parquet_path: str = DEFAULT_REGIME_PARQUET,
                      max_age: str = DEFAULT_MAX_AGE) -> np.ndarray:
    """
    timestamps: array-like of datetime64 (e.g. the npz dataset's
    ...
    # ns tz-naive normalization on BOTH sides (same helper as the val
    ...
    # BLOCKER-1 — index shift to AVAILABILITY TIME: the row labeled `t` holds
    ...
    # duplicated availability-time column: survives the merge → match age.
    right = df[GATE_COLS].reset_index().rename(
        columns={df.index.name or "index": "_t"})
    right["_avail"] = right["_t"]

    # BACKWARD = last AVAILABLE regime ≤ t (causal, never forward).
    ...
    # merge_asof sorts by _t — restore the original sample ordering.
    order = np.argsort(np.argsort(ts.values))
    G = G[order]
    age = age[order]

    # MAJOR-1 — bounded staleness: burn-in (no match → NaN) OR last regime
    ...
    # MINOR-A (B1 audit 2026-07-18) — fail-fast counts TOTAL fallbacks
