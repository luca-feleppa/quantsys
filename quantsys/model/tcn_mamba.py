...
Output:
  ...
  * loss_type="quantile":  (quantile_preds,) — (B, Q) with Q=5 standard quantiles
  * MoE: if n_output_experts > 1, softmax gate + experts (like QuantiTransformer)

2026-05-15: refactor to support quantile+MoE (previously hardcoded to t_student).
"""
import math
import torch
...
# Conv1d with left-only padding → temporal causality.
class CausalConv1d(nn.Module):
    ...
    """
    # Conv1d with pre-computed left padding to enforce causality.
    ...
    # Left-only padding → output at time t depends only on t' ≤ t.
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x = F.pad(x, (self.pad, 0))
        return self.conv(x)


# Residual TCN block with causal conv + GELU + DropPath.
class TCNBlock(nn.Module):
    ...
    """
    # Builds the residual block's causal conv + norm + GELU + dropout + DropPath.
    ...
    # Transpose for conv1d over T axis, then back. Residual + DropPath.
    def forward(self, x: torch.Tensor) -> torch.Tensor:
...
# TCN branch: dilations [1,2,4,8] → receptive field 31 steps (~30 min).
class TCNBranch(nn.Module):
    ...
    """
    # Builds the input projection + the dilated TCN blocks with increasing DropPath.
    def __init__(self, n_features: int, d_model: int, tcn_layers: int = 4,
        ...
        # Exponential dilations [1,2,4,..] + increasing DropPath per block.
    ...
    # Projects to d_model, applies TCN blocks, mean-pools over T.
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        h = self.proj(x)                        # (B, T, d_model)
        for blk in self.blocks:
            h = blk(h)
        return h.mean(dim=1)                    # global avg pool over T


# ─── Mamba Branch ────────────────────────────────────────────────────────────

# Pure-PyTorch simplified Mamba SSM (diagonal A → vectorized scan).
class SimplifiedMambaBlock(nn.Module):
    ...
    # Builds the Mamba block's SSM projections (in/B/C/dt), depthwise conv and log_A parameter.
    def __init__(self, d_model: int, d_state: int = 16, expand: int = 2,
        ...
        # log_A=0 → initial A=1; converges to A<1 during training.
        ...
        # input-dependent per-channel dt
    ...
    # dt bias s.t. softplus(bias) ≈ 0.01 → small initial steps.
    ...
    def _parallel_scan_chunk(A_bar_chunk: torch.Tensor,
        ...
        """
        # Promotes to FP32: cumprod/cumsum sensitive to FP16 underflow.
        orig_dtype = u_chunk.dtype
        A_f = A_bar_chunk.float()
        u_f = u_chunk.float()
        c_f = carry.float()

        # h_t = P_t·(carry + cumsum(u/P)_t) — vectorized closed form.
    ...
    # Mamba forward: selective SSM with gating + chunked parallel scan.
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        B, T, _ = x.shape
        residual = x
        x = self.norm(x)

        # in_proj split into (xb, z): z is the SiLU gating.
        xz     = self.in_proj(x)
        ...
        # Input-dependent B/C/dt (Mamba selectivity).
...
# Mamba branch: stack of SimplifiedMambaBlock; output = last token.
class MambaBranch(nn.Module):
    """Projects features, applies a stack of SimplifiedMambaBlocks, takes last token.

    Output: (B, d_model).
    """
    # Builds the input projection + the stack of SimplifiedMambaBlocks.
    ...
    # Projects to d_model, applies Mamba blocks, takes the last token.
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        h = self.proj(x)                        # (B, T, d_model)
        for blk in self.blocks:
            h = blk(h)
        return h[:, -1, :]                      # last-token pooling


# ─── Gated Fusion ────────────────────────────────────────────────────────────

# Gated fusion: σ(W·[tcn;mamba]) selects per-channel tcn vs mamba.
class GatedFusion(nn.Module):
    ...
    """
    # Builds the gate projection + LayerNorm for the fusion.
    def __init__(self, d_model: int):
        super().__init__()
        self.gate_proj = nn.Linear(d_model * 2, d_model)
        self.norm      = nn.LayerNorm(d_model)

    # Concatenates both representations, computes the gate, blends + norm.
...
# Hybrid TCN+Mamba architecture for probabilistic BTC/USDT 1-min forecasting.
class QuantTCNMamba(nn.Module):
    ...
    def __init__(
        ...
        # Builds TCN+Mamba branches, gated fusion and output heads.
        super().__init__()
        ...
        # ── Output heads ─────────────────────────────────────────────────────
        # Same pattern as QuantLSTM/QuantiTransformer: MoE → expert_gate+heads,
        ...
        # Spectral norm: only in single-expert t_student branch (legacy);
    ...
    # Inits mu/ls2/lnu heads (single-expert t_student branch only).
    def _init_output_heads(self):
        # Other branches init inline (quantile_head) or have no heads (MoE).
        if self.n_output_experts > 1 or self.loss_type == "quantile":
            return
        for head in (self.mu_head, self.ls2_head, self.lnu_head):
            nn.init.normal_(head.weight, std=0.01)
            nn.init.zeros_(head.bias)
        # softplus^-1(3)=log(e^3-1) → softplus(bias)+2 ≈ 5 degrees of freedom.
        with torch.no_grad():
            self.lnu_head.bias.fill_(math.log(math.expm1(3.0)))

    # No-op: this arch does not use clip bounds (train-loop compat only).
    ...
    # Forward: TCN + Mamba in parallel, fused via gating, then heads.
    def forward(self, x: torch.Tensor, x_macro=None,
                latent: torch.Tensor = None) -> tuple:
        # x_macro accepted for API uniformity but ignored (no macro here).
        ...
        # Output computation (MoE softmax gate or single head).
        if self.n_output_experts > 1:
