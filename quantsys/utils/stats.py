# Shared performance statistics — extracted VERBATIM from
...
# Bootstrap CI for Sharpe/Sortino — `annualize` = bars/year of the current timeframe
...
def bootstrap_sharpe_ci(pnl_list, n_boot=5000, confidence=0.95, annualize=525600):
    """Bootstrap confidence interval for Sharpe and Sortino (annualized at bars/year)."""
    if len(pnl_list) < 30:
...
# Duration and recovery time of the max drawdown along the equity curve.
def mdd_stats(equity_arr):
    """Computes duration and recovery time of the max drawdown."""
    ...
    # Recovery = first post-trough bar that recovers the previous peak.
    recovery_idx = len(eq) - 1  # default not recovered
