...
**Giudice FT1** — `python scripts/vol/ft1_execution_judge.py [--count-only]` (read-only, GPU-free). Implementa la pre-registrazione FT1 con l'emendamento 1 sul ledger di `--adaptive`; `NON AVVIATO` finché `adaptive.jsonl` non esiste. Il pull copia `adaptive.jsonl` (merge per chiave) e `adaptive_entry_journal.json` (presenza specchiata, come `position.json`). Output `results/vols/ft1_execution.json` (mai scritto da `--count-only`).
...
`scripts/06_dashboard.py` = **terminale opzioni crypto** (server HTTP single-file + SPA Plotly.js), GPU-free e **indipendente dalla pipeline ML**: legge i dati **pubblici Deribit** (REST, no-auth, nessuna chiave). Quattro tab — *Volatility Surface* (superficie IV 3D, smile, term structure ATM), *Option Chain* (chain call/put con Greche Black-Scholes forward), *Risk & Greeks* (OI per strike, max-pain, Greche aggregate pesate per OI, PCR, DVOL), *Trades* (forward test `04b`: storico + posizione aperta e profilo di payoff, endpoint `/api/trades` che legge `results/vol_paper/trades.jsonl` + `position.json` + `hedge_ledger.jsonl`. Legge ogni forma di record che 04b scrive — straddle v1, iron butterfly con premio netto e payoff a 4 gambe, uscita pin-close, tentativi strutturati `incomplete` mostrati senza posizione né PnL — e segnala una posizione con expiry passata e nessun record di settlement su disco come *expired, awaiting settlement* invece di *open*. PnL = gambe opzioni + gamba perp di hedge, quest'ultima ricostruita con `perp_leg` di `scripts/vol/hedged_vs_unhedged_judge.py` (stessa funzione, funding dalla cache locale `data/deribit_funding_perp.parquet`, nessuna rete); test `tests/test_dashboard_trades.py`). Auto-refresh ~12s. Config: `config/default.yaml → dashboard` — `host`/`port` (default `127.0.0.1:8050`), `options_currency` (BTC|ETH), `auth_token` opzionale (constant-time, header `X-Auth-Token` o `?token=`), `enable_gzip`.
